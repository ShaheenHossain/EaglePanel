...
class pluginInstaller:
    installLogPath = "/home/eaglepanel/modSecInstallLog"
    tempRulesFile = "/home/eaglepanel/tempModSecRules"
    mirrorPath = "eaglepanel.net"
    ...
    def migrationsEnabled(pluginName: str) -> bool:
        pluginHome = '/usr/local/EagleEP/' + pluginName
    ...
    def extractPlugin(pluginName):
        ...
        command = 'unzip ' + pathToPlugin + ' -d /usr/local/EagleEP'
        subprocess.call(shlex.split(command))

    @staticmethod
    def upgradingSettingsFile(pluginName):
        data = open("/usr/local/EagleEP/EagleEP/settings.py", 'r').readlines()
        writeToFile = open("/usr/local/EagleEP/EagleEP/settings.py", 'w')
    ...
    def upgradingURLs(pluginName):
        data = open("/usr/local/EagleEP/EagleEP/urls.py", 'r').readlines()
        writeToFile = open("/usr/local/EagleEP/EagleEP/urls.py", 'w')
    ...
    @staticmethod
    def informEaglePanel(pluginName):
        pluginPath = '/home/eaglepanel/plugins'
    ...
    def addInterfaceLink(pluginName):
        data = open("/usr/local/EagleEP/baseTemplate/templates/baseTemplate/index.html", 'r').readlines()
        writeToFile = open("/usr/local/EagleEP/baseTemplate/templates/baseTemplate/index.html", 'w')
    ...
    def staticContent():
        ...
        command = "rm -rf /usr/local/lscp/eaglepanel/static"
        subprocess.call(shlex.split(command))

        os.chdir('/usr/local/EagleEP')

        command = "/usr/local/EagleEP/bin/python manage.py collectstatic --noinput"
        subprocess.call(shlex.split(command))

        command = "mv /usr/local/EagleEP/static /usr/local/lscp/eaglepanel"
        subprocess.call(shlex.split(command))
    ...
    def installMigrations(pluginName):
        currentDir = os.getcwd()
        os.chdir('/usr/local/EagleEP')
        command = "/usr/local/EagleEP/bin/python manage.py makemigrations %s" % pluginName
        subprocess.call(shlex.split(command))
        command = "/usr/local/EagleEP/bin/python manage.py migrate %s" % pluginName
        subprocess.call(shlex.split(command))
        os.chdir(currentDir)
    ...
    def installPlugin(pluginName):
        try:
            ...
            pluginInstaller.stdOut('Informing EaglePanel about plugin.')
            pluginInstaller.informEaglePanel(pluginName)
            pluginInstaller.stdOut('EaglePanel core informed about the plugin.')
    ...
    def removeFiles(pluginName):
        pluginPath = '/usr/local/EagleEP/' + pluginName
    ...
    @staticmethod
    def informEaglePanelRemoval(pluginName):
        pluginPath = '/home/eaglepanel/plugins'
        pluginFile = pluginPath + '/' + pluginName
    ...
    def removeMigrations(pluginName):
        currentDir = os.getcwd()
        os.chdir('/usr/local/EagleEP')
        command = "/usr/local/EagleEP/bin/python manage.py migrate %s zero" % pluginName
        subprocess.call(shlex.split(command))
        os.chdir(currentDir)
    ...
    def removePlugin(pluginName):
        try:
            ...
            pluginInstaller.stdOut('Informing EaglePanel about plugin removal.')
            pluginInstaller.informEaglePanelRemoval(pluginName)
            pluginInstaller.stdOut('EaglePanel core informed about the plugin removal.')
