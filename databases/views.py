...
def generateAccess(request):
    try:
        ...
        keySavePath = '/home/eaglepanel/phpmyadmin_%s' % (admin.userName)
        try:
            GlobalUserDB.objects.get(username=admin.userName).delete()
        except:
            pass
...
def upgrademysqlnow(request):
    try:
        ...
        execPath = f"/usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/mysqlUtilities.py UpgradeMariaDB --version {version} --tempStatusPath {tempStatusPath}"
