...
class DNSManager:
    defaultNameServersPath = '/home/eaglepanel/defaultNameservers'
    ...
    def createNameserver(self, request = None, userID = None):
        ...
        if os.path.exists('/home/eaglepanel/powerdns'):
            finalData = {"status": 1}
        else:
            finalData = {"status": 0}
    ...
    def addDeleteDNSRecords(self, request = None, userID = None):
        ...
        if not os.path.exists('/home/eaglepanel/powerdns'):
            finalData = {"status": 0}
        else:
            finalData = {"status": 1}
    ...
    def installPowerDNS(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                try:
                    ...
                    if e.errno != errno.EEXIST and e.errno != errno.ENOENT:
                        logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], "[ERROR] Unable to rename /etc/resolv.conf to install PowerDNS: " +
                                                 str(e) + "[404]")
                        return 0
                    try:
            ...
            logging.EagleEPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPowerDNS]")
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
    ...
    def installPowerDNSConfigurations(self, mysqlPassword):
        ...
        #         os.remove(dnsPath)
        #         shutil.copy("/usr/local/EagleEP/install/dns-one/pdns.conf", dnsPath)
        #     else:
        #         shutil.copy("/usr/local/EagleEP/install/dns-one/pdns.conf", dnsPath)
        #
        ...
        try:
            ...
            cwd = os.getcwd()
            os.chdir('/usr/local/EagleEP/install')
            ...
            logging.EagleEPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPowerDNSConfigurations]")
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
    ...
    def ResetDNSConfigurations(self):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing DNS..,5')
            ...
            if self.installPowerDNSConfigurations(settings.DATABASES['default']['PASSWORD']) == 0:
                logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'installPowerDNSConfigurations failed. [404].')
                return 0
