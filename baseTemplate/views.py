...
def versionManagement(request):
    ...
    u = "https://api.github.com/repos/usmannasir/eaglepanel/commits?sha=v%s.%s" % (latestVersion, latestBuild)
    logging.writeToFile(u)
    r = requests.get(u)
    latestcomit = r.json()[0]['sha']

    command = "git -C /usr/local/EagleEP/ rev-parse HEAD"
...
@ensure_csrf_cookie
def upgrade_eaglepanel(request):
    if request.method == 'POST':
        try:
            upgrade_command = 'sh <(curl https://raw.githubusercontent.com/usmannasir/eaglepanel/stable/preUpgrade.sh || wget -O - https://raw.githubusercontent.com/usmannasir/eaglepanel/stable/preUpgrade.sh)'
            result = subprocess.run(upgrade_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                    text=True)

            if result.returncode == 0:
                response_data = {'success': True, 'message': 'EaglePanel upgrade completed successfully.'}
            else:
                response_data = {'success': False,
                                 'message': 'EaglePanel upgrade failed. Error output: ' + result.stderr}
...
def versionManagment(request):
    ...
    u = "https://api.github.com/repos/usmannasir/eaglepanel/commits?sha=v%s.%s" % (latestVersion, latestBuild)
    logging.EagleEPLogFileWriter.writeToFile(u)
    r = requests.get(u)
    latestcomit = r.json()[0]['sha']
...
def design(request):
    ### Load Custom CSS
    try:
        from baseTemplate.models import EaglePanelCosmetic
        cosmetic = EaglePanelCosmetic.objects.get(pk=1)
    except:
        from baseTemplate.models import EaglePanelCosmetic
        cosmetic = EaglePanelCosmetic()
        cosmetic.save()
    ...
    sha_url = "https://api.github.com/repos/usmannasir/EaglePanel-Themes/commits"

    sha_res = requests.get(sha_url)

    sha = sha_res.json()[0]['sha']

    l = "https://api.github.com/repos/usmannasir/EaglePanel-Themes/git/trees/%s" % sha
    fres = requests.get(l)
...
def getthemedata(request):
    try:
        ...
        # logging.EagleEPLogFileWriter.writeToFile(str(data) + "  [themedata]")

        url = "https://raw.githubusercontent.com/usmannasir/EaglePanel-Themes/main/%s/design.css" % data['Themename']
...
def runonboarding(request):
    try:
        ...
        command = f'/usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/virtualHostUtilities.py OnBoardingHostName --virtualHostName {hostname} --path {tempStatusPath} --rdns {rDNSCheck}'
