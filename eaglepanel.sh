...
#EaglePanel installer script for CentOS 7, CentOS 8, CloudLinux 7, AlmaLinux 8, RockyLinux 8, Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, openEuler 20.03 and openEuler 22.03
...
echo -e "Fetching latest data from EaglePanel server...\n"
...
Watchdog="On"
Redis_Hosting="No"
Temp_Value=$(curl --silent --max-time 30 -4 https://eaglepanel.net/version.txt)
...
LSWS_Latest_URL="https://eaglepanel.sh/update.litespeedtech.com/ws/latest.php"
...
Debug_Log() {
echo -e "\n${1}=${2}\n" >> "/var/log/eaglepanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
}

Debug_Log2() {
Check_Server_IP "$@" >/dev/null 2>&1
echo -e "\n${1}" >> /var/log/installLogs.txt
curl --max-time 20 -d '{"ipAddress": "'"$Server_IP"'", "InstallEaglePanelStatus": "'"$1"'"}' -H "Content-Type: application/json" -X POST https://cloud.eaglepanel.net/servers/RecvData  >/dev/null 2>&1
}
...
if [ -z "$XDG_CURRENT_DESKTOP" ]; then
    ...
    echo "$XDG_CURRENT_DESKTOP defined appears to be a desktop OS. Bailing as EaglePanel is incompatible."
    echo -e "\nEaglePanel is supported on server OS types only. Such as Ubuntu 18.04 x86_64, Ubuntu 20.04 x86_64, Ubuntu 20.10 x86_64, Ubuntu 22.04 x86_64, CentOS 8.x, AlmaLinux 8.x and CloudLinux 7.x...\n"
    exit
fi
...
if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
  ...
  echo -e "\nEaglePanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, CentOS 7, CentOS 8, CentOS 9, RHEL 8, RHEL 9, AlmaLinux 8, AlmaLinux 9, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  Debug_Log2 "EaglePanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, CentOS 7, CentOS 8, CentOS 9, RHEL 8, RHEL 9, AlmaLinux 8, RockyLinux 8, AlmaLinux 9, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03... [404]"
  exit
fi
...
#  echo -e "\nLXC detected..."
#  echo -e "EaglePanel does not support LXC"
#  echo -e "Exiting..."
#  Debug_Log2 "EaglePanel does not support LXC.. [404]"
#  exit
#fi
#remove per https://github.com/usmannasir/eaglepanel/issues/589
...
Show_Help() {
echo -e "\nEaglePanel Installer Script Help\n"
echo -e "\nUsage: sh <(curl eaglepanel.sh) --argument"
echo -e "\n\e[31m-v\e[39m or \e[31m--version\e[39m : choose to install EaglePanel OpenLiteSpeed or EaglePanel Enterprise, available options are \e[31mols\e[39m , \e[31mTRIAL\e[39m and \e[31mSERIAL_NUMBER\e[39m, default ols"
...
echo -e "\nExample:"
echo -e "\nsh <(curl eaglepanel.sh) -v ols -p r or ./eaglepanel.sh --version ols --password random"
echo -e "\nThis will install EaglePanel OpenLiteSpeed and randomly generate the password."
echo -e "\nsh <(curl eaglepanel.sh) -v LICENSE_KEY -a -p my_pass_word"
...
Interactive_Mode() {
echo -e "		EaglePanel Installer v$Panel_Version.$Panel_Build

1. Install EaglePanel.
...
1. Install EaglePanel with \e[31mOpenLiteSpeed\e[39m.

2. Install Eaglepanel with \e[31mLiteSpeed Enterprise\e[39m.
...
echo -e "\nInstall Full service for EaglePanel? This will include PowerDNS, Postfix and Pure-FTPd."
echo -e ""
...
if [[ $(expr "x$Tmp_Input" : 'x[Yy]') -gt 1 ]] || [[ $Tmp_Input = "" ]]; then
  ...
  ### Ask if you want to set up this EaglePanel with remote MySQL
...
if [[ $Server_OS = "CentOS" ]] ; then
  rpm --import https://eaglepanel.sh/rpms.litespeedtech.com/centos/RPM-GPG-KEY-litespeed
  ...
  if [[ "$Server_OS_Version" = "9" ]]; then
    ...
    yum install -y https://eaglepanel.sh/dl.fedoraproject.org/pub/epel/epel-release-latest-9.noarch.rpm
...
EOF
  ...
  if [[ "$Server_OS_Version" = "8" ]]; then
    rpm --import https://eaglepanel.sh/www.centos.org/keys/RPM-GPG-KEY-CentOS-Official
    rpm --import https://eaglepanel.sh/dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-8
    yum install -y https://eaglepanel.sh/dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
      Check_Return "yum repo" "no_exit"
...
#    cat <<EOF >/etc/yum.repos.d/CentOS-PowerTools-EaglePanel.repo
#[powertools-for-eaglepanel]
...
#EOF
  ...
  if [[ "$Server_OS_Version" = "7" ]]; then
    rpm --import https://eaglepanel.sh/dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-7
    ...
    yum install -y yum-plugin-priorities
      Check_Return "yum repo" "no_exit"
    curl -o /etc/yum.repos.d/powerdns-auth-43.repo https://eaglepanel.sh/repo.powerdns.com/repo-files/centos-auth-43.repo
      Check_Return "yum repo" "no_exit"
...
EOF
    ...
    yum install --nogpg -y https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/gf-release-latest.gf.el7.noarch.rpm
      Check_Return "yum repo" "no_exit"

    rpm -ivh https://eaglepanel.sh/repo.iotti.biz/CentOS/7/noarch/lux-release-7-1.noarch.rpm
      Check_Return "yum repo" "no_exit"

    rpm -ivh https://eaglepanel.sh/repo.ius.io/ius-release-el7.rpm
      Check_Return "yum repo" "no_exit"
  fi
fi
...
[global]
index-url=https://eaglepanel.sh/pip-repo/pypi/simple/
EOF
#default to self-host pip for CN

if [[ "$Server_Provider" = "Alibaba Cloud" ]] ; then
sed -i 's|https://eaglepanel.sh/pip-repo/pypi/simple/|http://mirrors.cloud.aliyuncs.com/pypi/simple/|g' /root/.pip/pip.conf
echo "trusted-host = mirrors.cloud.aliyuncs.com" >> /root/.pip/pip.conf
fi

if [[ "$Server_Provider" = "Tencent Cloud" ]] ; then
sed -i 's|https://eaglepanel.sh/pip-repo/pypi/simple/|https://mirrors.cloud.tencent.com/pypi/simple/|g' /root/.pip/pip.conf
fi
...
if [[ "$Server_OS" = "CentOS" ]] && [[ "$Server_OS_Version" = "7" ]]; then
  ...
  sed -i 's|http://yum.mariadb.org|https://eaglepanel.sh/yum.mariadb.org|g' /etc/yum.repos.d/MariaDB.repo
  sed -i 's|https://yum.mariadb.org/RPM-GPG-KEY-MariaDB|https://eaglepanel.sh/yum.mariadb.org/RPM-GPG-KEY-MariaDB|g' /etc/yum.repos.d/MariaDB.repo
  # use MariaDB Mirror

  sed -i 's|https://download.copr.fedorainfracloud.org|https://eaglepanel.sh/download.copr.fedorainfracloud.org|g' /etc/yum.repos.d/_copr_copart-restic.repo

  sed -i 's|http://repo.iotti.biz|https://eaglepanel.sh/repo.iotti.biz|g' /etc/yum.repos.d/frank.repo

  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/7/gf/\$basearch/mirrorlist|baseurl=https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/el/7/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/7/plus/\$basearch/mirrorlist|baseurl=https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/el/7/plus/x86_64/|g" /etc/yum.repos.d/gf.repo

  sed -i 's|https://repo.ius.io|https://eaglepanel.sh/repo.ius.io|g' /etc/yum.repos.d/ius.repo

  sed -i 's|http://repo.iotti.biz|https://eaglepanel.sh/repo.iotti.biz|g' /etc/yum.repos.d/lux.repo

  sed -i 's|http://repo.powerdns.com|https://eaglepanel.sh/repo.powerdns.com|g' /etc/yum.repos.d/powerdns-auth-43.repo
  sed -i 's|https://repo.powerdns.com|https://eaglepanel.sh/repo.powerdns.com|g' /etc/yum.repos.d/powerdns-auth-43.repo
fi
#  sed -i 's|http://mirrors.tencentyun.com/ubuntu/|https://eaglepanel.sh/us.archive.ubuntu.com/ubuntu/|g' /etc/apt/sources.list
...
python3 -m venv /usr/local/EaglePanel
Check_Return
else
virtualenv -p /usr/bin/python3 /usr/local/EaglePanel
  Check_Return
fi

if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" != "20" ]] ; then
  # shellcheck disable=SC1091
  source /usr/local/EaglePanel/bin/activate
else
  # shellcheck disable=SC1091
  . /usr/local/EaglePanel/bin/activate
fi
...
if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "22" ]] ; then
  cp /usr/bin/python3.10 /usr/local/EagleEP/bin/python3
fi

rm -rf eaglepanel
echo -e "\nFetching files from ${Git_Clone_URL}...\n"

Debug_Log2 "Getting EaglePanel code..,4"

Retry_Command "git clone ${Git_Clone_URL}"
  Check_Return "git clone ${Git_Clone_URL}"

echo -e "\nEaglePanel source code downloaded...\n"

cd eaglepanel || exit
git checkout "$Branch_Name"
  Check_Return "git checkout"
cd - || exit
cp -r eaglepanel /usr/local/eaglepanel
cd eaglepanel/install || exit
...
if [[ "$Server_OS" = "CentOS" ]] ; then
  ...
  #disable SELinux
    ...
    if [[ "$Server_OS_Version" = "8" ]] ; then
        ...
        if [[ "$Server_Country" = "CN" ]] ; then
          sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://eaglepanel.sh/litespeed/litespeed_cn.repo|g' install.py
        else
          sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://eaglepanel.sh/litespeed/litespeed.repo|g' install.py
        fi
      fi
    fi
...
if ! grep -q "pid_max" /etc/rc.local 2>/dev/null ; then
  ...
  *                hard    nproc           65535
  ...
  # Check Connectivity
  if ping -q -c 1 -W 1 eaglepanel.sh >/dev/null; then
...
Line1="$(grep -n "f.write('nameserver 8.8.8.8')" installEaglePanel.py | head -n 1 | cut -d: -f1)"
sed -i "${Line1}i\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ subprocess.call\(command, shell=True)" installEaglePanel.py
sed -i "${Line1}i\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ command = 'cat /etc/resolv.conf-tmp > /etc/resolv.conf'" installEaglePanel.py
}
...
if [ -f /root/eaglepanel-tmp ]; then
  rm -rf /root/eaglepanel-tmp
fi

mkdir /root/eaglepanel-tmp
cd /root/eaglepanel-tmp || exit

Retry_Command "wget https://eaglepanel.sh/www.litespeedtech.com/packages/${LSWS_Stable_Version:0:1}.0/lsws-$LSWS_Stable_Version-ent-x86_64-linux.tar.gz"
tar xzvf "lsws-$LSWS_Stable_Version-ent-x86_64-linux.tar.gz" >/dev/null
cd "/root/eaglepanel-tmp/lsws-$LSWS_Stable_Version/conf"  || exit
if [[ "$License_Key" = "Trial" ]]; then
  Retry_Command "wget -q https://eaglepanel.sh/license.litespeedtech.com/reseller/trial.key"
  sed -i "s|writeSerial = open('lsws-6.0/serial.no', 'w')|command = 'wget -q --output-document=./lsws-$LSWS_Stable_Version/trial.key https://eaglepanel.sh/license.litespeedtech.com/reseller/trial.key'|g" "$Current_Dir/installEaglePanel.py"
  sed -i 's|writeSerial.writelines(self.serial)|subprocess.call(command, shell=True)|g' "$Current_Dir/installEaglePanel.py"
  sed -i 's|writeSerial.close()||g' "$Current_Dir/installEaglePanel.py"
else
  echo "$License_Key" > serial.no
fi

cd "/root/eaglepanel-tmp/lsws-$LSWS_Stable_Version/bin"  || exit
...
if [[ "$Server_OS" = "Ubuntu" ]] ; then
  sed -i 's|wget http://rpms.litespeedtech.com/debian/|wget https://eaglepanel.sh/litespeed/|g' install.py
  sed -i 's|https://repo.dovecot.org/|https://eaglepanel.sh/repo.dovecot.org/|g' install.py
fi
  #replace litespeed repo on ubuntu 18/20

if [[ "$Server_OS" = "CentOS" ]] ; then
  sed -i 's|rpm -ivh http://rpms.litespeedtech.com/centos/litespeed-repo-1.2-1.el7.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://eaglepanel.sh/litespeed/litespeed_cn.repo|g' install.py
  sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://eaglepanel.sh/litespeed/litespeed_cn.repo|g' install.py
  sed -i 's|https://mirror.ghettoforge.org/distributions|https://eaglepanel.sh/mirror.ghettoforge.org/distributions|g' install.py
  ...
  sed -i 's|dnf --nogpg install -y https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/gf-release-latest.gf.el8.noarch.rpm|echo gf8|g' install.py

  Retry_Command "dnf --nogpg install -y https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/gf-release-latest.gf.el8.noarch.rpm"
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/gf/\$basearch/mirrorlist|baseurl=https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/el/8/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/plus/\$basearch/mirrorlist|baseurl=https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/el/8/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
  ...
  if [[ "$Server_OS_Version" = "9" ]] ; then
    ...
    sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/gf/\$basearch/mirrorlist|baseurl=https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/el/9/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
    sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/plus/\$basearch/mirrorlist|baseurl=https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/el/9/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
  fi
fi

sed -i "s|https://www.litespeedtech.com/|https://eaglepanel.sh/www.litespeedtech.com/|g" installEaglePanel.py
...
sed -i 's|http://www.litespeedtech.com|https://eaglepanel.sh/www.litespeedtech.com|g' install.py
sed -i 's|https://snappymail.eu/repository/latest.tar.gz|https://eaglepanel.sh/www.snappymail.eu/repository/latest.tar.gz|g' install.py

sed -i "s|rep.eaglepanel.net|eaglepanel.sh/rep.eaglepanel.net|g" installEaglePanel.py
sed -i "s|rep.eaglepanel.net|eaglepanel.sh/rep.eaglepanel.net|g" install.py
...
sed -i 's|$PROJECT/archive/$BRANCH.tar.gz|https://eaglepanel.sh/codeload.github.com/acmesh-official/acme.sh/tar.gz/master|g' /root/.acme.sh/acme.sh
...
if [[ -d /usr/local/EagleEP ]] ; then
  echo -e "\n EaglePanel already installed, exiting..."
  Debug_Log2 "EaglePanel already installed, exiting... [404]"
  exit
fi
...
if [[ $Server_Edition = "Enterprise" ]] ; then
  ...
  sed -i "s|lsws-5.4.2|lsws-$LSWS_Stable_Version|g" installEaglePanel.py
  sed -i "s|lsws-5.3.5|lsws-$LSWS_Stable_Version|g" installEaglePanel.py
  sed -i "s|lsws-6.0|lsws-$LSWS_Stable_Version|g" installEaglePanel.py
  #this sed must be done after license validation

  Enterprise_Flag="--ent ent --serial "
fi

sed -i 's|git clone https://github.com/usmannasir/eaglepanel|echo downloaded|g' install.py
sed -i 's|mirror.eaglepanel.net|eaglepanel.sh|g' install.py
...
/usr/local/EaglePanel/bin/python install.py "${Final_Flags[@]}"


if grep "EaglePanel installation successfully completed" /var/log/installLogs.txt >/dev/null; then
  echo -e "\nEaglePanel installation sucessfully completed...\n"
...
if [[ $Server_OS = "CentOS" ]] || [[ $Server_OS = "openEuler" ]]; then
  ...
  wget -O lsmcd-master.zip https://eaglepanel.sh/codeload.github.com/litespeedtech/lsmcd/zip/master
...
chmod +x /usr/local/EagleEP/CPScripts/setup_php_sessions.sh
bash /usr/local/EagleEP/CPScripts/setup_php_sessions.sh
...
Current_Dir="$(pwd)"
rm -f /usr/local/lsws/eaglepanel-tmp
mkdir /usr/local/lsws/eaglepanel-tmp
cd /usr/local/lsws/eaglepanel-tmp || exit
wget -O timezonedb.tgz https://eaglepanel.sh/pecl.php.net/get/timezonedb
...
rm -rf /usr/local/lsws/eaglepanel-tmp
cd "$Current_Dir" || exit
...
echo "${Webadmin_Pass}" >/etc/eaglepanel/webadmin_passwd
chmod 600 /etc/eaglepanel/webadmin_passwd
}

Post_Install_Setup_Watchdog() {
if [[ "$Watchdog" = "On" ]]; then
  wget -O /etc/eaglepanel/watchdog.sh "${Git_Content_URL}/stable/CPScripts/watchdog.sh"
  chmod 700 /etc/eaglepanel/watchdog.sh
  ln -s /etc/eaglepanel/watchdog.sh /usr/local/bin/watchdog
...
snappymailAdminPass=$(grep SetPassword /usr/local/EagleEP/public/snappymail.php| sed -e 's|$oConfig->SetPassword(||g' -e "s|');||g" -e "s|'||g")
Elapsed_Time="$((Time_Count / 3600)) hrs $(((SECONDS / 60) % 60)) min $((Time_Count % 60)) sec"
echo "###################################################################"
echo "                EaglePanel Successfully Installed                  "
echo "                                                                   "
...
echo "                                                                   "
echo -e "             Run \e[31meaglepanel help\e[39m to get FAQ info"
echo -e "             Run \e[31meaglepanel upgrade\e[39m to upgrade it to latest version."
echo -e "             Run \e[31meaglepanel utility\e[39m to access some handy tools ."
echo "                                                                   "
echo "              Website : https://www.eaglepanel.net                 "
echo "              Forums  : https://forums.eaglepanel.net              "
echo "              Wikipage: https://docs.eaglepanel.net                "
echo "              Docs    : https://eaglepanel.net/docs/               "
echo "                                                                   "
echo -e "            Enjoy your accelerated Internet by                  "
echo -e "                EaglePanel & $Word 				                     "
echo "###################################################################"
...
echo -e "\033[0;32mTCP: 8090\033[39m for EaglePanel"
...
cat <<EOF >/root/eaglepanel/cert_conf
[req]
prompt=no
distinguished_name=eaglepanel
[eaglepanel]
commonName = www.example.com
countryName = CP
localityName = EaglePanel
organizationName = EaglePanel
organizationalUnitName = EaglePanel
stateOrProvinceName = CP
emailAddress = mail@example.com
name = EaglePanel
surname = EaglePanel
givenName = EaglePanel
initials = CP
dnQualifier = EaglePanel
[server_exts]
extendedKeyUsage = 1.3.6.1.5.5.7.3.1
EOF
openssl req -x509 -config /root/eaglepanel/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout /usr/local/lscp/conf/key.pem -out /usr/local/lscp/conf/cert.pem
...
openssl req -x509 -config /root/eaglepanel/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout "$Key_Path" -out "$Cert_Path"
rm -f /root/eaglepanel/cert_conf
}
...
if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "22" ]] ; then
python3 -m venv /usr/local/EagleEP
Check_Return
else
virtualenv -p /usr/bin/python3 /usr/local/EagleEP
  Check_Return
fi

if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "20" ]] ; then
  # shellcheck disable=SC1091
  . /usr/local/EagleEP/bin/activate
   Check_Return
else
  # shellcheck disable=SC1091
  source /usr/local/EagleEP/bin/activate
   Check_Return
...
chown -R eaglepanel:eaglepanel /usr/local/EagleEP/lib
chown -R eaglepanel:eaglepanel /usr/local/EagleEP/lib64 || true
}

Pre_Install_Setup_Git_URL() {
if [[ $Server_Country != "CN" ]] ; then
  Git_User="usmannasir"
  Git_Content_URL="https://raw.githubusercontent.com/${Git_User}/eaglepanel"
  Git_Clone_URL="https://github.com/${Git_User}/eaglepanel.git"
else
  Git_User="qtwrk"
  Git_Content_URL="https://gitee.com/${Git_User}/eaglepanel/raw"
  Git_Clone_URL="https://gitee.com/${Git_User}/eaglepanel.git"
fi
...
sed -i "s|lsws-5.3.8|lsws-$LSWS_Stable_Version|g" /usr/local/EagleEP/serverStatus/serverStatusUtil.py
sed -i "s|lsws-5.4.2|lsws-$LSWS_Stable_Version|g" /usr/local/EagleEP/serverStatus/serverStatusUtil.py
sed -i "s|lsws-5.3.5|lsws-$LSWS_Stable_Version|g" /usr/local/EagleEP/serverStatus/serverStatusUtil.py
...
/usr/local/EaglePanel/bin/python /usr/local/EagleEP/plogical/adminPass.py --password "$Admin_Pass"
mkdir -p /etc/opendkim

echo '/usr/local/EaglePanel/bin/python /usr/local/EagleEP/plogical/adminPass.py --password $@' > /usr/bin/adminPass
echo "systemctl restart lscpd" >> /usr/bin/adminPass
echo "echo \$@ > /etc/eaglepanel/adminPass" >> /usr/bin/adminPass
...
#fix php.ini &amp; issue
  ...
  #all centos 7 specific post change goes here
    ...
    if [[ ! -f /usr/local/lsws/lsphp74/lib64/php/modules/zip.so ]] ; then
      ...
      yum install -y https://eaglepanel.sh/misc/libzip-0.11.2-6.el7.psychotic.x86_64.rpm
      yum install -y https://eaglepanel.sh/misc/libzip-devel-0.11.2-6.el7.psychotic.x86_64.rpm
...
HostName=$(hostname --fqdn); [ -n "$(dig @1.1.1.1 +short "$HostName")" ]  &&  echo "$HostName resolves to valid IP. Setting up hostname SSL" && eaglepanel createWebsite --package Default --owner admin --domainName $(hostname --fqdn) --email root@localhost --php 7.4 && eaglepanel hostNameSSL --domainName $(hostname --fqdn)


}

Post_Install_CN_Replacement() {
sed -i 's|wp core download|wp core download https://eaglepanel.sh/wordpress.org/latest.tar.gz|g' /usr/local/EagleEP/plogical/applicationInstaller.py
sed -i 's|https://raw.githubusercontent.com/|https://eaglepanel.sh/raw.githubusercontent.com/|g' /usr/local/EagleEP/plogical/applicationInstaller.py
sed -i 's|wp plugin install litespeed-cache|wp plugin install  https://eaglepanel.sh/downloads.wordpress.org/plugin/litespeed-cache.zip|g' /usr/local/EagleEP/plogical/applicationInstaller.py

sed -i 's|https://www.litespeedtech.com/|https://eaglepanel.sh/www.litespeedtech.com/|g' /usr/local/EagleEP/serverStatus/serverStatusUtil.py
sed -i 's|http://license.litespeedtech.com/|https://eaglepanel.sh/license.litespeedtech.com/|g' /usr/local/EagleEP/serverStatus/serverStatusUtil.py
}
...
if [[ "$*" = *"--debug"* ]] ; then
  Debug="On"
  find /var/log -name 'eaglepanel_debug_*' -exec rm {} +
  Random_Log_Name=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 5)
  echo -e "$(date)" > "/var/log/eaglepanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
  chmod 600 "/var/log/eaglepanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
fi
