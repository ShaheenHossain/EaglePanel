...
def litespeedStatus(request):
    try:
        ...
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[litespeedStatus]")
            proc = httpProc(request, 'serverStatus/litespeedStatus.html',
                            {"processList": processList,
                             "liteSpeedVersionStatus": "For some reaons not able to load version details, see EagleEP main log file.",
    ...
    try:
        ...
        if reboot == 1:
            if installUtilities.reStartLiteSpeedSocket() == 1:
                ...
                status = {"reboot": 0, "shutdown": 0, "error_message": "Please see EagleEP main log file."}
        else:
        ...
        logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[stopOrRestartLitespeed]")
        return HttpResponse("Not Logged in as admin")


def cyberCPMainLogFile(request):
    proc = httpProc(request, 'serverStatus/eagleepmainlogfile.html', None, 'admin')
    return proc.render()
...
def getFurtherDataFromLogFile(request):
    try:
        ...
        fewLinesOfLogFile = logging.EagleEPLogFileWriter.readLastNFiles(50, logging.EagleEPLogFileWriter.fileName)
        ...
        status = {"logstatus": 0,
                  "error": "Could not fetch data from log file, please see EagleEP main log file through command line."}
        logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[getFurtherDataFromLogFile]")
        return HttpResponse("Not Logged in as admin")
...
def switchTOLSWS(request):
    try:
        ...
        execPath = "sudo /usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/serverStatus/serverStatusUtil.py"
...
def changeLicense(request):
    try:
        ...
        try:
            ...
            command = 'sudo chown -R eaglepanel:eaglepanel /usr/local/lsws/conf'
            ProcessUtilities.executioner(command)
...
def topProcessesStatus(request):
    try:
        ...
        with open("/home/eaglepanel/top", "w") as outfile:
            subprocess.call("top -n1 -b", shell=True, stdout=outfile)

        data = open('/home/eaglepanel/top', 'r').readlines()
        ...
        data['EaglePanelVersion'] = 'v%s.%s' % (VERSION, str(BUILD))
    ...
    try:
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
            ...
            if type == 'EaglePanel':
                ...
                command = 'cat /usr/local/EagleEP/AllCPUbuntu.json'
            ...
            if type == 'installed':
                ...
                for items in packages:
                    if items == '':
                        ...
                        startForUpdate = startForUpdate + 1
            elif type == 'EaglePanel':
                command = 'cat /usr/local/EagleEP/CPCent7repo.json'
                packages = json.loads(ProcessUtilities.outputExecutioner(command))

        ## make list of packages that need update

        # if os.path.exists(ProcessUtilities.debugPath):
        #    logging.EagleEPLogFileWriter.writeToFile('All packages: %s' % (str(packages)))
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.EagleEPLogFileWriter.writeToFile('Final packages: %s' % (str(finalPackages)))
        ...
        for items in finalPackages:
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                try:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile('[ERROR] %s. [fetchPackages:773]' % (str(msg)))
            elif ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                try:
                    ...
                    print(str(msg))
                    logging.EagleEPLogFileWriter.writeToFile('[ERROR] %s. [fetchPackages:839]' % (str(msg)))
...
def lockStatus(request):
    try:
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
            ...
            yumConfTmp = '/home/eaglepanel/yumTemp'
...
def EaglePanelPort(request):
    port = ProcessUtilities.fetchCurrentPort()
    proc = httpProc(request, "serverStatus/changeEaglePanelPort.html", {'port': port}, 'admin')
    return proc.render()
