...
class ServerStatusUtil(multi.Thread):
    ...
    lswsInstallStatusPath = '/home/eaglepanel/switchLSWSStatus'
    ...
    def installLiteSpeed(licenseKey, statusFile):
        try:
            ...
            if os.path.exists('/usr/local/EagleEP/lsws-6.0/'):
                shutil.rmtree('/usr/local/EagleEP/lsws-6.0')

            if os.path.exists('/usr/local/EagleEP/lsws-6.2/'):
                shutil.rmtree('/usr/local/EagleEP/lsws-6.2/')

            if ACLManager.ISARM():
                command = 'tar zxf lsws-6.2-ent-aarch64-linux.tar.gz -C /usr/local/EagleEP'
            else:
                command = 'tar zxf lsws-6.2-ent-x86_64-linux.tar.gz -C /usr/local/EagleEP'

            if ServerStatusUtil.executioner(command, statusFile) == 0:
                return 0

            if licenseKey == 'trial':
                command = 'wget -q --output-document=/usr/local/EagleEP/lsws-6.2/trial.key http://license.litespeedtech.com/reseller/trial.key'
                if ServerStatusUtil.executioner(command, statusFile) == 0:
                    return 0
            else:
                writeSerial = open('/usr/local/EagleEP/lsws-6.2/serial.no', 'w')
                writeSerial.writelines(licenseKey)
                writeSerial.close()

            shutil.copy('/usr/local/EagleEP/serverStatus/litespeed/install.sh', '/usr/local/EagleEP/lsws-6.2/')
            shutil.copy('/usr/local/EagleEP/serverStatus/litespeed/functions.sh', '/usr/local/EagleEP/lsws-6.2/')

            os.chdir('/usr/local/EagleEP/lsws-6.2/')
            ...
            shutil.copy('/usr/local/EagleEP/serverStatus/litespeed/httpd_config.xml', confPath)
            shutil.copy('/usr/local/EagleEP/serverStatus/litespeed/modsec.conf', confPath)
            shutil.copy('/usr/local/EagleEP/serverStatus/litespeed/httpd.conf', confPath)
            ...
            try:
                os.rmdir("/usr/local/EagleEP/lsws-6.2")
            except:
                pass
    ...
    def setupFileManager(statusFile):
        try:
            logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, "Setting up Filemanager files..\n")
            ...
            shutil.copytree("/usr/local/EagleEP/serverStatus/litespeed/FileManager",fileManagerPath)
            ...
            logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,"Filemanager files are set!\n")
    ...
    def rebuildvConf():
        try:
            ...
            for website in allWebsites:
                logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,
    ...
    def switchTOLSWS(licenseKey):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,"Starting conversion process..\n")
            logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,
            ...
            if ServerStatusUtil.installLiteSpeed(licenseKey, statusFile) == 0:
                logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, "Failed to install LiteSpeed. [404]", 1)
            ...
            #     logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, "Failed to set up File Manager. [404]", 1)
            #     ServerStatusUtil.recover()
            #     return 0
            ...
            logging.EagleEPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,"Successfully switched to LITESPEED ENTERPRISE WEB SERVER. [200]\n", 1)
