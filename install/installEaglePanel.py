...
class InstallEaglePanel:
    ...
    def OSFlags():
        if os.path.exists("/etc/redhat-release"):
            ...
            if data.find('CloudLinux 8') > -1 or data.find('cloudlinux 8') > -1:
                InstallEaglePanel.CloudLinux8 = 1
        ...
        if self.ent == 0:
            ...
            try:
                ...
                if self.serial == '1111-2222-3333-4444':
                    command = 'wget -q --output-document=/root/eaglepanel/install/lsws-6.2/trial.key http://license.litespeedtech.com/reseller/trial.key'
                    install.preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
                else:
    ...
    def fix_ols_configs(self):
        try:
            ...
            InstallEaglePanel.stdOut("Fixing OpenLiteSpeed configurations!", 1)
            ...
            InstallEaglePanel.stdOut("OpenLiteSpeed Configurations fixed!", 1)
    ...
    def changePortTo80(self):
        try:
            InstallEaglePanel.stdOut("Changing default port to 80..", 1)
            ...
            InstallEaglePanel.stdOut("Default port is now 80 for OpenLiteSpeed!", 1)
    ...
    def installAllPHPVersions(self):
        ...
        InstallEaglePanel.stdOut("LiteSpeed PHPs successfully installed!", 1)
...
"""
        ...
        if self.remotemysql == 'OFF':
            if self.distro == ubuntu:
                passwordCMD = "use mysql;DROP DATABASE IF EXISTS test;DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%%';GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' IDENTIFIED BY '%s';UPDATE user SET plugin='' WHERE User='root';flush privileges;" % (
                    InstallEaglePanel.mysql_Root_password)
            else:
    ...
    def fixMariaDB(self):
        self.stdOut("Setup MariaDB so it can support Eaglepanel's needs")
        ...
        self.stdOut("MariaDB is now setup so it can support Eaglepanel's needs")
    ...
    def installPureFTPD(self):
        if self.distro == ubuntu:
            ...
            if get_Ubuntu_release() == 18.10:
                command = 'wget https://rep.eaglepanel.net/pure-ftpd-common_1.0.47-3_all.deb'
                install.preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

                command = 'wget https://rep.eaglepanel.net/pure-ftpd-mysql_1.0.47-3_amd64.deb'
                install.preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
    ...
    def installPureFTPDConfigurations(self, mysql):
        try:
            ...
            InstallEaglePanel.stdOut("Configuring PureFTPD..", 1)
            ...
            dataWritten = "MYSQLPassword " + InstallEaglePanel.mysqlPassword + '\n'
            for items in data:
            ...
            InstallEaglePanel.stdOut("PureFTPD configured!", 1)
            ...
            if self.distro == ubuntu or self.distro == cent8 or self.distro == openeuler:
                ...
                try:
                    ...
                    if e.errno != errno.EEXIST and e.errno != errno.ENOENT:
                        InstallEaglePanel.stdOut("[ERROR] Unable to rename /etc/resolv.conf to install PowerDNS: " +
                                                 str(e), 1, 1, os.EX_OSERR)
                    try:
                        os.remove('/etc/resolv.conf')
                    except OSError as e1:
                        InstallEaglePanel.stdOut(
                ...
                #     f.close()
                # except IOError as e:
                #     InstallEaglePanel.stdOut("[ERROR] Unable to create /etc/resolv.conf: " + str(e) +
    ...
    def installPowerDNSConfigurations(self, mysqlPassword, mysql):
        try:
            ...
            InstallEaglePanel.stdOut("Configuring PowerDNS..", 1)
            ...
            InstallEaglePanel.stdOut("PowerDNS configured!", 1)
...
def Main(cwd, mysql, distro, ent, serial=None, port="8090", ftp=None, dns=None, publicip=None, remotemysql=None,
    ...
    InstallEaglePanel.mysqlPassword = randomPassword.generate_pass()
    InstallEaglePanel.mysql_Root_password = randomPassword.generate_pass()

    file_name = '/etc/eaglepanel/mysqlPassword'

    if remotemysql == 'OFF':
        if os.access(file_name, os.F_OK):
            password = open(file_name, 'r')
            InstallEaglePanel.mysql_Root_password = password.readline()
            password.close()
        else:
            password = open(file_name, "w")
            password.writelines(InstallEaglePanel.mysql_Root_password)
            password.close()
    else:
    ...
    try:
        ...
        install.preFlightsChecks.call(command, distro, '[chmod]',
                                      '',
                                      1, 0, os.EX_OSERR)
        command = 'chown root:eaglepanel %s' % (file_name)
        install.preFlightsChecks.call(command, distro, '[chmod]',
                                      '',
                                      1, 0, os.EX_OSERR)
    except:
    ...
    if distro == centos:
        ...
        InstallEaglePanel.mysqlPassword = InstallEaglePanel.mysql_Root_password

    installer = InstallEaglePanel("/usr/local/lsws/", cwd, distro, ent, serial, port, ftp, dns, publicip, remotemysql,
    ...
    mysqlUtilities.createDatabase("eaglepanel", "eaglepanel", InstallEaglePanel.mysqlPassword, publicip)
    ...
    if dns is None:
        ...
        installer.installPowerDNSConfigurations(InstallEaglePanel.mysqlPassword, mysql)
