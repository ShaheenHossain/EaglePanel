...
class preFlightsChecks:
    debug = 1
    eaglePanelMirror = "mirror.eaglepanel.net/pip"
    cdn = 'eaglepanel.sh'
    SnappyVersion = '2.33.0'

    def __init__(self, rootPath, ip, path, cwd, eaglePanelPath, distro, remotemysql=None, mysqlhost=None, mysqldb=None,
                 mysqluser=None, mysqlpassword=None, mysqlport=None):
        self.ipAddr = ip
        self.path = path
        self.cwd = cwd
        self.server_root_path = rootPath
        self.eaglePanelPath = eaglePanelPath
        self.distro = distro
    ...
    def setup_account_eaglepanel(self):
        try:
            ...
            if self.distro == ubuntu:
                self.stdOut("Add Eaglepanel user")
                command = 'adduser --disabled-login --gecos "" eaglepanel'
                preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

            else:
                command = "useradd -s /bin/false eaglepanel"
                preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
            ...
            command = 'usermod -aG docker eaglepanel'
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            ...
            logging.InstallLog.writeToFile("[ERROR] setup_account_eaglepanel. " + str(msg))

    def installEaglePanelRepo(self):
        self.stdOut("Install Eaglepanel repo")
        ...
        if self.distro == ubuntu:
            try:
                ...
                logging.InstallLog.writeToFile("[ERROR] Exception during EaglePanel install")
                preFlightsChecks.stdOut("[ERROR] Exception during EaglePanel install")
    ...
    def download_install_EaglePanel(self, mysqlPassword, mysql):
        ##
        ...
        command = "git clone https://github.com/usmannasir/eaglepanel"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

        shutil.move('eaglepanel', 'EagleEP')
        ...
        if self.remotemysql == 'OFF':
            passFile = "/etc/eaglepanel/mysqlPassword"
...
""" % password
        ...
        path = self.eaglePanelPath + "/EagleEP/settings.py"
        ...
        os.chdir("/usr/local/EagleEP")

        command = "/usr/local/EaglePanel/bin/python manage.py makemigrations"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

        ##

        command = "/usr/local/EaglePanel/bin/python manage.py migrate"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

        if not os.path.exists("/usr/local/EagleEP/public"):
            os.mkdir("/usr/local/EagleEP/public")

        command = "/usr/local/EaglePanel/bin/python manage.py collectstatic --noinput --clear"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

        ## Moving static content to lscpd location
        command = 'mv static /usr/local/EagleEP/public/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)

        try:
            path = "/usr/local/EagleEP/version.txt"
    ...
    def fixEaglePanelPermissions(self):

        ###### fix Core EaglePanel permissions
        ...
        command = "find /usr/local/EagleEP -type d -exec chmod 0755 {} \;"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "find /usr/local/EagleEP -type f -exec chmod 0644 {} \;"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chmod -R 755 /usr/local/EagleEP/bin"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        ## change owner

        command = "chown -R root:root /usr/local/EagleEP"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        command = "chown -R lscpd:lscpd /usr/local/EagleEP/public/phpmyadmin/tmp"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        command = "chown -R lscpd:lscpd /usr/local/lscp/eaglepanel/rainloop"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chmod 700 /usr/local/EagleEP/cli/eaglePanel.py"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chmod 700 /usr/local/EagleEP/plogical/upgradeCritical.py"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chmod 755 /usr/local/EagleEP/postfixSenderPolicy/client.py"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chmod 640 /usr/local/EagleEP/EagleEP/settings.py"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chown root:eaglepanel /usr/local/EagleEP/EagleEP/settings.py"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        fileM = ['/usr/local/lsws/FileManager/', '/usr/local/EagleEP/install/FileManager',
                 '/usr/local/EagleEP/serverStatus/litespeed/FileManager', '/usr/local/lsws/Example/html/FileManager']
        ...
        command = 'chmod +x /usr/local/EagleEP/plogical/renew.py'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'chmod +x /usr/local/EagleEP/CLManager/CLPackages.py'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        clScripts = ['/usr/local/EagleEP/CLScript/panel_info.py', '/usr/local/EagleEP/CLScript/CloudLinuxPackages.py',
                     '/usr/local/EagleEP/CLScript/CloudLinuxUsers.py',
                     '/usr/local/EagleEP/CLScript/CloudLinuxDomains.py',
                     '/usr/local/EagleEP/CLScript/CloudLinuxResellers.py', '/usr/local/EagleEP/CLScript/CloudLinuxAdmins.py',
                     '/usr/local/EagleEP/CLScript/CloudLinuxDB.py', '/usr/local/EagleEP/CLScript/UserInfo.py']

        for items in clScripts:
            command = 'chmod +x %s' % (items)
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'chmod 600 /usr/local/EagleEP/plogical/adminPass.py'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'chmod 600 /etc/cagefs/exclude/eaglepanelexclude'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "find /usr/local/EagleEP/ -name '*.pyc' -delete"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        command = 'chmod 640 /usr/local/lscp/eaglepanel/logs/access.log'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'mkdir -p/usr/local/lscp/eaglepanel/snappymail/data/_data_/_default_/configs/'

        snappymailinipath = '/usr/local/lscp/eaglepanel/snappymail/data/_data_/_default_/configs/application.ini'

        command = 'chmod 600 /usr/local/EagleEP/public/snappymail.php'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        command = 'chmod 700 %s' % ('/home/eaglepanel')
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def download_install_phpmyadmin(self):
        try:
            ...
            command = 'wget -O /usr/local/EagleEP/public/phpmyadmin.zip https://github.com/usmannasir/eaglepanel/raw/stable/phpmyadmin.zip'

            preFlightsChecks.call(command, self.distro, '[download_install_phpmyadmin]',
                                  command, 1, 0, os.EX_OSERR)

            command = 'unzip /usr/local/EagleEP/public/phpmyadmin.zip -d /usr/local/EagleEP/public'
            preFlightsChecks.call(command, self.distro, '[download_install_phpmyadmin]',
                                  command, 1, 0, os.EX_OSERR)

            command = 'mv /usr/local/EagleEP/public/phpMyAdmin-*-all-languages /usr/local/EagleEP/public/phpmyadmin'
            subprocess.call(command, shell=True)

            command = 'rm -f /usr/local/EagleEP/public/phpmyadmin.zip'
            preFlightsChecks.call(command, self.distro, '[download_install_phpmyadmin]',
                                  command, 1, 0, os.EX_OSERR)
            ...
            data = open('/usr/local/EagleEP/public/phpmyadmin/config.sample.inc.php', 'r').readlines()

            writeToFile = open('/usr/local/EagleEP/public/phpmyadmin/config.inc.php', 'w')
...
"""
            ...
            writeToFile.writelines("$cfg['TempDir'] = '/usr/local/EagleEP/public/phpmyadmin/tmp';\n")

            writeToFile.close()

            os.mkdir('/usr/local/EagleEP/public/phpmyadmin/tmp')

            command = 'chown -R lscpd:lscpd /usr/local/EagleEP/public/phpmyadmin'
            preFlightsChecks.call(command, self.distro, '[chown -R lscpd:lscpd /usr/local/EagleEP/public/phpmyadmin]',
                                  'chown -R lscpd:lscpd /usr/local/EagleEP/public/phpmyadmin', 1, 0, os.EX_OSERR)

            if self.remotemysql == 'ON':
                command = "sed -i 's|'localhost'|'%s'|g' %s" % (
                    self.mysqlhost, '/usr/local/EagleEP/public/phpmyadmin/config.inc.php')
                preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            command = 'cp /usr/local/EagleEP/plogical/phpmyadminsignin.php /usr/local/EagleEP/public/phpmyadmin/phpmyadminsignin.php'
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            if self.remotemysql == 'ON':
                command = "sed -i 's|localhost|%s|g' /usr/local/EagleEP/public/phpmyadmin/phpmyadminsignin.php" % (
                    self.mysqlhost)
                preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setup_email_Passwords(self, mysqlPassword, mysql):
        try:
            ...
            if mysql == 'Two':
                dataWritten = "connect = host=127.0.0.1 dbname=eaglepanel user=eaglepanel password=" + mysqlPassword + " port=3307\n"
            else:
                dataWritten = "connect = host=localhost dbname=eaglepanel user=eaglepanel password=" + mysqlPassword + " port=3306\n"
    ...
    def downoad_and_install_raindloop(self):
        try:
            ...
            if os.path.exists("/usr/local/EagleEP/public/snappymail"):
                return 0

            os.chdir("/usr/local/EagleEP/public")
            ...
            command = 'unzip snappymail-%s.zip -d /usr/local/EagleEP/public/snappymail' % (preFlightsChecks.SnappyVersion)
            preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
            ...
            os.chdir("/usr/local/EagleEP/public/snappymail")
            ...
            command = "mkdir -p /usr/local/lscp/eaglepanel/rainloop/data"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            ### Enable sub-folders

            command = "mkdir -p /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/configs/"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

#             labsPath = '/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/configs/application.ini'
#
#             labsData = """[labs]
...
#             # writeToFile.close()
#
#             iPath = os.listdir('/usr/local/EagleEP/public/snappymail/snappymail/v/')
#
#             path = "/usr/local/EagleEP/public/snappymail/snappymail/v/%s/include.php" % (iPath[0])
#
#             data = open(path, 'r').readlines()
#             writeToFile = open(path, 'w')
#
...
#                     writeToFile.writelines(
#                         "			$sCustomDataPath = '/usr/local/lscp/eaglepanel/rainloop/data';\n")
#                 else:
#                     writeToFile.writelines(items)
#
#             writeToFile.close()
#
#             includeFileOldPath = '/usr/local/EagleEP/public/snappymail/_include.php'
#             includeFileNewPath = '/usr/local/EagleEP/public/snappymail/include.php'
#
#             if os.path.exists(includeFileOldPath):
#                 writeToFile = open(includeFileOldPath, 'a')
#                 writeToFile.write("\ndefine('APP_DATA_FOLDER_PATH', '/usr/local/lscp/eaglepanel/rainloop/data/');\n")
#                 writeToFile.close()
#
...
#
#             command = f'mkdir /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
#
#             command = f'chmod 700 /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
#
#             command = f'chmod 700 /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
#
#             command = f'wget -O /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect/index.php https://raw.githubusercontent.com/the-djmaze/snappymail/master/plugins/mailbox-detect/index.php'
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
#
#             command = f'chmod 644 /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect/index.php'
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
#
#             command = f'chown lscpd:lscpd /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect/index.php'
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
#
...
#             PluginsFilePath = '/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/configs/plugin-mailbox-detect.json'
#
...
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            ...
            command = f'wget -O /usr/local/EagleEP/snappymail_eaglepanel.php  https://raw.githubusercontent.com/the-djmaze/snappymail/master/integrations/eaglepanel/install.php'
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            command = f'/usr/local/lsws/lsphp80/bin/php /usr/local/EagleEP/snappymail_eaglepanel.php'
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def installLSCPD(self):
        try:
            ...
            command = f'cp -f /usr/local/EagleEP/{lscpdSelection} /usr/local/lscp/bin/{lscpdSelection}'
            preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
            ...
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            try:
                os.mkdir('/usr/local/lscp/eaglepanel')
            except:
                pass
            try:
                os.mkdir('/usr/local/lscp/eaglepanel/logs')
            except:
                pass
    ...
    def setupComodoRules(self):
        try:
            ...
            command = "wget https://eaglepanel.net/modsec/comodo.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setupPythonWSGI(self):
        try:
            ...
            command = "/usr/local/EaglePanel/bin/python ./configure.py"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            ...
            if not os.path.exists('/usr/local/EagleEP/bin/'):
                os.mkdir('/usr/local/EagleEP/bin/')

            command = "cp lswsgi /usr/local/EagleEP/bin/"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setupLSCPDDaemon(self):
        try:
            ...
            path = "/usr/local/EagleEP/conf/"
            command = "mkdir -p " + path
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            path = "/usr/local/EagleEP/conf/token_env"
            writeToFile = open(path, "w")
    ...
    def setup_cron(self):
        ...
        try:
            ...
            content = """
0 * * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/findBWUsage.py >/dev/null 2>&1
0 * * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/postfixSenderPolicy/client.py hourlyCleanup >/dev/null 2>&1
0 0 1 * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/postfixSenderPolicy/client.py monthlyCleanup >/dev/null 2>&1
0 2 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/upgradeCritical.py >/dev/null 2>&1
0 0 * * 4 /usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/renew.py >/dev/null 2>&1
7 0 * * * "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" > /dev/null
0 0 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py Daily
0 0 * * 0 /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py Weekly

*/30 * * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '30 Minutes'
0 * * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '1 Hour'
0 */6 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '6 Hours'
0 */12 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '12 Hours'
0 1 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '1 Day'
0 0 */3 * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '3 Days'
0 0 * * 0 /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py '1 Week'
...
"""
    ...
    def install_default_keys(self):
        try:
            ...
            command = "ssh-keygen -f /root/.ssh/eaglepanel -t rsa -N ''"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def test_Requests(self):
        try:
            import requests
            getVersion = requests.get('https://eaglepanel.net/version.txt')
            ...
            command = "pip install http://mirror.eaglepanel.net/urllib3-1.22.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            command = "pip install http://mirror.eaglepanel.net/requests-2.18.4.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

    def installation_successfull(self):
        print("###################################################################")
        print("                EaglePanel Successfully Installed                  ")
        print("                                                                   ")
...
"""
    ...
    def setupCLI(self):
        command = "ln -s /usr/local/EagleEP/cli/eaglePanel.py /usr/bin/eaglepanel"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = "chmod +x /usr/local/EagleEP/cli/eaglePanel.py"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def enableDisableDNS(state):
        try:
            servicePath = '/home/eaglepanel/powerdns'
    ...
    def enableDisableEmail(state):
        try:
            servicePath = '/home/eaglepanel/postfix'
    ...
    def enableDisableFTP(state, distro):
        try:
            servicePath = '/home/eaglepanel/pureftpd'
    ...
    def setUpFirstAccount():
        try:
            command = 'python /usr/local/EagleEP/plogical/adminPass.py --password 1234567'
            subprocess.call(shlex.split(command))
        except:
            pass
...
panel_info = /usr/local/EagleEP/CLScript/panel_info.py
packages = /usr/local/EagleEP/CLScript/CloudLinuxPackages.py
users = /usr/local/EagleEP/CLScript/CloudLinuxUsers.py
domains = /usr/local/EagleEP/CLScript/CloudLinuxDomains.py
resellers = /usr/local/EagleEP/CLScript/CloudLinuxResellers.py
admins = /usr/local/EagleEP/CLScript/CloudLinuxAdmins.py
db_info = /usr/local/EagleEP/CLScript/CloudLinuxDB.py

[lvemanager_config]
ui_user_info =/usr/local/EagleEP/CLScript/UserInfo.py
...
"""
                ...
                writeToFile = open('/etc/cagefs/exclude/eaglepanelexclude', 'w')
                writeToFile.write(content)
                writeToFile.close()
...
def main():
    ...
    logging.InstallLog.writeToFile("Starting EaglePanel installation..,10")
    preFlightsChecks.stdOut("Starting EaglePanel installation..")
    ...
    try:
        os.mkdir("/etc/eaglepanel")
    except:
        pass

    machineIP = open("/etc/eaglepanel/machineIP", "w")
    ...
    distro = get_distro()
    checks = preFlightsChecks("/usr/local/lsws/", args.publicip, "/usr/local", cwd, "/usr/local/EagleEP", distro,
    ...
    checks.setup_account_eaglepanel()
    checks.installEaglePanelRepo()

    import installEaglePanel

    if ent == 0:
        installEaglePanel.Main(cwd, mysql, distro, ent, None, port, args.ftp, args.powerdns, args.publicip, remotemysql,
                               mysqlhost, mysqldb, mysqluser, mysqlpassword, mysqlport)
    else:
        installEaglePanel.Main(cwd, mysql, distro, ent, serial, port, args.ftp, args.powerdns, args.publicip,
                               remotemysql, mysqlhost, mysqldb, mysqluser, mysqlpassword, mysqlport)
    ...
    if args.postfix is None:
        ...
        checks.setup_email_Passwords(installEaglePanel.InstallEaglePanel.mysqlPassword, mysql)
    ...
    checks.download_install_EaglePanel(installEaglePanel.InstallEaglePanel.mysqlPassword, mysql)
    ...
    try:
        # command = 'mkdir -p /usr/local/lscp/eaglepanel/snappymail/data/data/default/configs/'
        # subprocess.call(shlex.split(command))

        writeToFile = open('/usr/local/lscp/eaglepanel/snappymail/data/_data_/_default_/configs/application.ini', 'a')
...
include '/usr/local/EagleEP/public/snappymail/index.php';
...
?>""" % (randomPassword.generate_pass())
        ...
        writeToFile = open('/usr/local/EagleEP/public/snappymail.php', 'w')
        writeToFile.write(content)
        writeToFile.close()

        command = '/usr/local/lsws/lsphp72/bin/php /usr/local/EagleEP/public/snappymail.php'
        subprocess.call(shlex.split(command))

        command = "chown -R lscpd:lscpd /usr/local/lscp/eaglepanel/snappymail/data"
        subprocess.call(shlex.split(command))
    except:
        pass

    checks.fixEaglePanelPermissions()

    logging.InstallLog.writeToFile("EaglePanel installation successfully completed!,80")
