...
def getLogsFromFile(request):
    try:
        ...
        status = {"status": 0, "logstatus":0,"error":"Could not fetch data from log file, please see EagleEP main log file through command line."}
        logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[getLogsFromFile]")
        final_json = json.dumps(status)
        return HttpResponse(final_json)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                execPath = "/usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/serverLogs.py"
