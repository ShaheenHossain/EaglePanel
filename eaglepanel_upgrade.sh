...
echo -e "\nFetching latest data from EaglePanel server...\n"
...
MySQL_Password=$(cat /etc/eaglepanel/mysqlPassword)
...
rm -rf /root/eaglepanel_upgrade_tmp
mkdir -p /root/eaglepanel_upgrade_tmp
cd /root/eaglepanel_upgrade_tmp || exit
}

Debug_Log() {
echo -e "\n${1}=${2}\n" >>  "/var/log/eaglepanel_debug_upgrade_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
}
...
echo -e "\nChecking root privileges..."
  ...
  if [[ $(id -u) != 0 ]] >/dev/null; then
    echo -e "\nYou must run as root user to install EaglePanel...\n"
...
if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
  ...
  echo -e "\nEaglePanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, CentOS 7, CentOS 8, AlmaLinux 8, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  Debug_Log2 "EaglePanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, CentOS 7, CentOS 8, AlmaLinux 8, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03... [404]"
  exit
fi
...
Regenerate_Cert() {
  cat <<EOF >/usr/local/EagleEP/cert_conf
[req]
prompt=no
...
EOF
  if [[ $1 == "8090" ]]; then
    openssl req -x509 -config /usr/local/EagleEP/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout /usr/local/lscp/conf/key.pem -out /usr/local/lscp/conf/cert.pem
  fi
  ...
  if [[ $1 == "7080" ]]; then
    ...
    openssl req -x509 -config /usr/local/EagleEP/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout $key_path -out $cert_path
  fi
  rm -f /usr/local/EagleEP/cert_conf
}
...
if [[ "$Server_OS" = "CentOS" ]] ; then
  rm -f /etc/yum.repos.d/EaglePanel.repo
  rm -f /etc/yum.repos.d/litespeed.repo
  if [[ "$Server_Country" = "CN" ]] ; then
    curl -o /etc/yum.repos.d/litespeed.repo https://eaglepanel.sh/litespeed/litespeed_cn.repo
  else
    curl -o /etc/yum.repos.d/litespeed.repo https://eaglepanel.sh/litespeed/litespeed.repo
  fi
  yum clean all
  yum update -y
...
#EOF
  rm -f /etc/yum.repos.d/CentOS-PowerTools-EaglePanel.repo

  if [[ "$Server_Country" = "CN" ]] ; then
    dnf --nogpg install -y https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/gf-release-latest.gf.el8.noarch.rpm
  else
  ...
  if [[ "$Server_Country" = "CN" ]] ; then
    dnf --nogpg install -y https://eaglepanel.sh/mirror.ghettoforge.org/distributions/gf/gf-release-latest.gf.el9.noarch.rpm
  else
...
if [[ -f /usr/local/EaglePanel/bin/python2 ]]; then
  echo -e "\nPython 2 dectected, doing re-setup...\n"
  rm -rf /usr/local/EaglePanel/bin
  virtualenv -p /usr/bin/python3 --system-site-packages /usr/local/EaglePanel
  Check_Return
elif [[ -d /usr/local/EaglePanel/bin/ ]]; then
  echo -e "\nNo need to re-setup virtualenv at /usr/local/EaglePanel...\n"
else
...
Main_Upgrade() {
/usr/local/EaglePanel/bin/python upgrade.py "$Branch_Name"
  Check_Return

if [[ -f /usr/local/EagleEP/bin/python2 ]]; then
  rm -rf /usr/local/EagleEP/bin
  virtualenv -p /usr/bin/python3 /usr/local/EagleEP
    Check_Return
elif [[ -d /usr/local/EagleEP/bin/ ]]; then
  echo -e "\nNo need to re-setup virtualenv at /usr/local/EagleEP...\n"
else
  virtualenv -p /usr/bin/python3 --system-site-packages /usr/local/EagleEP
    Check_Return
fi
...
wget https://eaglepanel.sh/www.litespeedtech.com/packages/lsapi/wsgi-lsapi-2.1.tgz
tar xf wsgi-lsapi-2.1.tgz
cd wsgi-lsapi-2.1 || exit
/usr/local/EaglePanel/bin/python ./configure.py
make

rm -f /usr/local/EagleEP/bin/lswsgi
cp lswsgi /usr/local/EagleEP/bin/
...
if [[ -f /etc/eaglepanel/webadmin_passwd ]]; then
...
if [[ -f /etc/eaglepanel/watchdog.sh ]] ; then
	watchdog kill
	rm -f /etc/eaglepanel/watchdog.sh
	rm -f /usr/local/bin/watchdog
	wget -O /etc/eaglepanel/watchdog.sh "${Git_Content_URL}/${Branch_Name}/CPScripts/watchdog.sh"
...
chown -R eaglepanel:eaglepanel /usr/local/EagleEP/lib64
...
if [[ ! -d /etc/eaglepanel ]] ; then
  echo -e "\n\nCan not detect EagleEP..."
  exit
fi
...
if [[ "$*" = *"--debug"* ]] ; then
  ...
  find /var/log -name 'eaglepanel_debug_upgrade_*' -exec rm {} +
  echo -e "$(date)" > "/var/log/eaglepanel_debug_upgrade_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
  chmod 600 "/var/log/eaglepanel_debug_upgrade_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
fi
