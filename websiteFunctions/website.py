...
class WebsiteManager:
    ...
    def WordpressPricing(self, request=None, userID=None, ):
        Data = {}
        proc = httpProc(request, 'websiteFunctions/EaglepanelPricing.html', Data, 'createWebsite')
        return proc.render()
    ...
    def SearchOnkeyupPlugin(self, userID=None, data=None):
        try:
            if ACLManager.CheckForPremFeature('wp-manager'):
                ...
                # logging.EagleEPLogFileWriter.writeToFile("Plugin Name ....... %s"%pluginname)
    ...
    def AddNewpluginAjax(self, userID=None, data=None):
        try:
            ...
            Name = data['Name']
            # pluginname = data['pluginname']
            # logging.EagleEPLogFileWriter.writeToFile("config ....... %s"%config)
            # logging.EagleEPLogFileWriter.writeToFile(" Name ....... %s"%Name)
    ...
    def deletesPlgin(self, userID=None, data=None, ):
        try:
            ...
            # logging.EagleEPLogFileWriter.writeToFile("pluginbID ....... %s" % pluginbBucketID)
            # logging.EagleEPLogFileWriter.writeToFile("pluginname ....... %s" % pluginname)
    ...
    def DeploytoProduction(self, userID=None, data=None):
        try:
            ...
            extraArgs['tempStatusPath'] = "/home/eaglepanel/" + str(randint(1000, 9999))
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile("Creating Bucket Error: %s" % str(msg))
                    data_ret = {'status': 0, 'error_message': str(msg)}
                    json_data = json.dumps(data_ret)
                    return HttpResponse(json_data)
        ...
        try:
            ...
            for web in allweb:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile(result)
    ...
    def ChangeStatusThemes(self, userID=None, data=None):
        try:
            # logging.EagleEPLogFileWriter.writeToFile("Error WP ChangeStatusThemes ....... %s")
            currentACL = ACLManager.loadedACL(userID)
            admin = Administrator.objects.get(pk=userID)
            ...
            if setting == 'lscache':
                ...
                if settingValue:
                    command = "sudo -u %s %s -d error_reporting=0 /usr/bin/wp config set WP_DEBUG true --path=%s --skip-plugins --skip-themes" % (
                        Vhuser, FinalPHPPath, path)
                    stdoutput = ProcessUtilities.outputExecutioner(command)
                    logging.EagleEPLogFileWriter.writeToFile("Debugging mk true 1  output:" + str(stdoutput))

                    command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp config list --skip-plugins --skip-themes --path=%s' % (
                        Vhuser, FinalPHPPath, path)
                    stdout = ProcessUtilities.outputExecutioner(command)
                    logging.EagleEPLogFileWriter.writeToFile("Debugging output:" + str(stdout))
                    ...
                    command = "sudo -u %s %s -d error_reporting=0 /usr/bin/wp config set WP_DEBUG false --path=%s --skip-plugins --skip-themes" % (
                        Vhuser, FinalPHPPath, path)
                    stdoutput = ProcessUtilities.outputExecutioner(command)
                    logging.EagleEPLogFileWriter.writeToFile("Debugging mk false 0  output:" + str(stdoutput))
                ...
                execPath = f"/usr/local/EagleEP/bin/python {virtualHostUtilities.eaglePanel}/plogical/virtualHostUtilities.py"
    ...
    def submitWebsiteCreation(self, userID=None, data=None):
        try:
            ...
            if data['domainName'].find("eaglepanel.website") > -1:
                url = "https://platform.cyberpersons.com/EaglepanelAdOns/CreateDomain"
            ...
            tempStatusPath = "/home/eaglepanel/" + str(randint(1000, 9999))
    ...
    def fetchWebsitesList(self, userID=None, data=None):
        try:
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Fetch sites step 1..')

            endPageNumber, finalPageNumber = self.recordsPointer(pageNumber, recordsToShow)

            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Fetch sites step 2..')

            websites = ACLManager.findWebsiteObjects(currentACL, userID)

            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Fetch sites step 3..')

            pagination = self.getPagination(len(websites), recordsToShow)

            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Fetch sites step 4..')

            json_data = self.findWebsitesListJson(websites[finalPageNumber:endPageNumber])

            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Fetch sites step 5..')
    ...
    def findWebsitesListJson(self, websites):
        ...
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile("Failed to read machine IP, error:" + str(msg))
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.EagleEPLogFileWriter.writeToFile(f'findWebsitesListJson 1')

        for items in websites:
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'findWebsitesListJson 2')
            if items.state == 0:
                state = "Suspended"
            else:
                state = "Active"
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(vhFile)
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'findWebsitesListJson 3')

            DiskUsage, DiskUsagePercentage, bwInMB, bwUsage = virtualHostUtilities.FindStats(items)
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'findWebsitesListJson 4')
            try:
    ...
    def submitWebsiteDeletion(self, userID=None, data=None):
        try:
            if data['websiteName'].find("eaglepanel.website") > -1:
                url = "https://platform.cyberpersons.com/EaglepanelAdOns/DeleteDomain"
    ...
    def saveWebsiteChanges(self, userID=None, data=None):
        try:
            ...
            ## Fix https://github.com/usmannasir/eaglepanel/issues/998

            # from plogical.IncScheduler import IncScheduler
            # isPU = IncScheduler('CalculateAndUpdateDiskUsage', {})
            # isPU.start()

            command = '/usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/IncScheduler.py UpdateDiskUsageForce'
    ...
    def getDataFromLogFile(self, userID=None, data=None):
        ...
        if output.find("1,None") > -1:
            final_json = json.dumps(
                {'status': 0, 'logstatus': 0,
                 'error_message': "Not able to fetch logs, see EaglePanel main log file, Error: %s" % (output)})
            return HttpResponse(final_json)
    ...
    def fetchErrorLogs(self, userID=None, data=None):
        ...
        if output.find("1,None") > -1:
            final_json = json.dumps(
                {'status': 0, 'logstatus': 0, 'error_message': "Not able to fetch logs, see EaglePanel main log file!"})
            return HttpResponse(final_json)
    ...
    def saveConfigsToFile(self, userID=None, data=None):
        ...
        if ProcessUtilities.outputExecutioner(command).find('127.0.0.1') == -1:
            ...
            tempPath = "/home/eaglepanel/" + str(randint(1000, 9999))
    ...
    def saveSSL(self, userID=None, data=None):
        ...
        tempKeyPath = "/home/eaglepanel/" + str(randint(1000, 9999))
        vhost = open(tempKeyPath, "w")
        vhost.write(key)
        vhost.close()

        tempCertPath = "/home/eaglepanel/" + str(randint(1000, 9999))
        ...
        try:
            ...
            for alias in website.childdomains_set.filter(alais=1):
                ...
                try:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile(f'Error changing PHP for alias: {str(msg)}')
    ...
    def getWebsiteCron(self, userID=None, data=None):
        try:
            ...
            execPath = "/usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/cronUtil.py"
    ...
    def setupGit(self, request=None, userID=None, data=None):
        ...
        path = '/home/eaglepanel/' + self.domain + '.git'
...
""" % (self.domain, website.externalApp)
    ...
    def tuneSettings(self, userID=None, data=None):
        try:
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'PHP path in tune settings {phpPath}')
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'PHP Version in tune settings {phpVersion}')

            phpService = ApacheVhost.DecideFPMServiceName(phpVersion)

            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'PHP service in tune settings {phpService}')
    ...
    def folderCheck(self):
        ...
        try:
            ...
            gitConfFolder = '/home/eaglepanel/git'
            ...
            logging.EagleEPLogFileWriter.writeToFile('%s. [folderCheck:3002]' % (str(msg)))
    ...
    def deleteSSHKey(self, userID=None, data=None):
        try:
            ...
            execPath = "/usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/firewallUtilities.py"
    ...
    def addSSHKey(self, userID=None, data=None):
        try:
            ...
            execPath = "sudo /usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/firewallUtilities.py"
    ...
    def fetchDockersite(self, userID=None, data=None):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile("Our dockersite" + str(dockersites))
