#!/bin/bash
#EaglePanel utility script

export LC_CTYPE=en_US.UTF-8
SUDO_TEST=$(set)
BRANCH_NAME="stable"
GIT_URL="github.com/usmannasir/eaglepanel"
GIT_CONTENT_URL="raw.githubusercontent.com/usmannasir/eaglepanel"
...
if [[ $TMP_YN == "1" ]] ; then
	if [[ -f /etc/eaglepanel/watchdog.sh ]] ; then
		bash /etc/eaglepanel/watchdog.sh kill
	fi
		rm -f /etc/eaglepanel/watchdog.sh
		rm -f /usr/local/bin/watchdog
		wget -O /etc/eaglepanel/watchdog.sh https://$GIT_CONTENT_URL/$BRANCH_NAME/CPScripts/watchdog.sh
...
self_check() {
echo -e "\nChecking Eaglepanel Utility update..."
SUM=$(md5sum /usr/bin/eaglepanel_utility)
SUM1=${SUM:0:32}
...
SUM=$(md5sum /usr/local/EaglePanel/eaglepanel_utility.sh)
SUM2=${SUM:0:32}
#get md5sum of remote file.

if [[ $SUM1 == $SUM2 ]] ; then
	echo -e "\nEaglePanel Utility Script is up to date...\n"
else
	local_string=$(head -2 /usr/bin/eaglepanel_utility)
	remote_string=$(head -2 /usr/local/EaglePanel/eaglepanel_utility.sh)
	#check file content before replacing itself in case failed to download the file.
	if [[ $local_string == $remote_string ]] ; then
	echo -e "\nUpdating EaglePanel Utility Script..."
	rm -f /usr/bin/eaglepanel_utility
	mv /usr/local/EaglePanel/eaglepanel_utility.sh /usr/bin/eaglepanel_utility
	chmod 700 /usr/bin/eaglepanel_utility
	echo -e "\nEaglePanel Utility update compelted..."
...
if [[ $SERVER_COUNTRY == "CN" ]] ; then
	GIT_URL="gitee.com/qtwrk/eaglepanel"
	GIT_CONTENT_URL="gitee.com/qtwrk/eaglepanel/raw"
fi

#echo -e "EaglePanel Upgrade will start in 10 seconds"
#echo -e "If you want to cancel, please press CTRL + C to cancel it"
#sleep 10
echo -e "EaglePanel upgrading..."
rm -f /usr/local/eaglepanel_upgrade.sh
wget -O /usr/local/eaglepanel_upgrade.sh -q https://$GIT_CONTENT_URL/${BRANCH_NAME}/eaglepanel_upgrade.sh
...
curl --silent https://eaglepanel.sh/misc/faq.sh | sudo -u nobody bash | less -r
exit
}
	...
	if [[ $TMP_YN == "1" ]] ; then
		...
		if [[ -f /usr/local/lsmcd/bin/lsmcd ]] ; then
			...
			if [[ $SERVER_OS == "CentOS" ]] || [[ $SERVER_OS == "openEuler" ]] ; then
				...
				wget https://cdn.eaglepanel.sh/litespeed/lsmcd.tar.gz
...
main_page() {
echo -e "		EaglePanel Utility Tools \e[31m(beta)\e[39m

  1. Upgrade EaglePanel.
...
panel_check(){
if [[ ! -f /etc/eaglepanel/machineIP ]] ; then
	echo -e "\nCan not detect EaglePanel..."
	echo -e "\nExit..."
	exit
fi
}
...
sudo_check() {
	...
	if [[ $(id -u) != 0 ]]  > /dev/null; then
		echo -e "\nYou must use root user to use EaglePanel Utility..."
		exit
	else
