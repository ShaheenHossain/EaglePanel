...
class IncJobs(multi.Thread):
    ...
    def findRestorePath(self):
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8 \
            ...
            if self.jobid.type[:8] == 'database':
                self.restoreTarget = '/usr/local/EagleEP/tmp/'
    ...
    def getAWSData(self):
        ...
        path = '/home/eaglepanel/aws/%s' % (key)
    ...
    def restoreDatabase(self):
        try:
            ...
            if self.reconstruct == 'remote':
                ...
                command = 'chown eaglepanel:eaglepanel /home/eaglepanel'
                ProcessUtilities.executioner(command)

                command = 'chmod 755 /home/eaglepanel'
                ProcessUtilities.executioner(command)

                ##

                if mysqlUtilities.mysqlUtilities.restoreDatabaseBackup(self.path.split('/')[-1].rstrip('.sql'),
                                                                       '/usr/local/EagleEP/tmp', 'dummy', 'dummy') == 0:
                    raise BaseException
            else:
            ...
            try:
                if self.reconstruct == 'remote':
                    os.remove('/usr/local/EagleEP/tmp/%s' % (self.path.split('/')[-1]))
                else:
                    os.remove('/usr/local/EagleEP/tmp/%s.sql' % (self.jobid.type.split(':')[1]))
    ...
    def reconstructWithMeta(self):
        try:
            ...
            execPath = "nice -n 10 /usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/restoreMeta.py"
    ...
    def backupDatabases(self):
        try:
            ...
            for items in databases:
                ...
                try:
                    dbPath = '/usr/local/EagleEP/tmp/%s.sql' % (items.dbName)
...
""" % (password)
        ...
        sender = 'eaglepanel@%s' % (self.website.domain)
            ...
            if ProcessUtilities.outputExecutioner(command).find('restic is a backup program which') == -1:
                try:
                    ...
                    logging.statusWriter(self.statusPath,
                                         ...
                                         'guide -> https://go.eaglepanel.net/restic. [5009]', 1)
                    pass
