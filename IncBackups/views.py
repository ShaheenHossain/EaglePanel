...
def add_destination(request):
    try:
        ...
        if data['type'].lower() == IncBackupProvider.SFTP.name.lower():
            ...
            python_path = Path('/usr/local/EagleEP/bin/python')
            backup_utils = Path(vhu.eaglePanel) / "plogical/backupUtilities.py"
...
def submit_backup_creation(request):
    try:
        ...
        temp_path = Path("/home/eaglepanel/") / str(randint(1000, 9999))
...
def get_backup_status(request):
    try:
        ...
        if os.path.exists(status):
            command = "cat " + status
            result = pu.outputExecutioner(command, 'eaglepanel')
...
def selectwebsiteRetorev2(request):
    ...
    try:
        ...
        #/home/eaglepanel.net/.config/rclone/rclone.conf
