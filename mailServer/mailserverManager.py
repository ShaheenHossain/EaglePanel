...
try:
    ...
    pass
import plogical.EagleEPLogFileWriter as logging
import json
import shlex
import subprocess
try:
...
class MailServerManager(multi.Thread):
    ...
    def run(self):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + ' [MailServerManager.run]')
    ...
    def createEmailAccount(self):
        ...
        if not os.path.exists('/home/eaglepanel/postfix'):
        ...
        try:
            ...
            if result[0] == 1:
                ...
                if EmailLimits != -1:
                    ...
                    RandomFile = "/home/eaglepanel/" + str(randint(100000, 999999))
                    writeToFile = open(RandomFile, 'w')
                    writeToFile.write(limitString)
                    writeToFile.close()

                    execPath = "/usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/mailUtilities.py"
    ...
    def fixMailSSL(self, data = None):
        try:
            ...
            execPath = "/usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/virtualHostUtilities.py"
    ...
    def RunServerLevelEmailChecks(self):
        try:
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Checking if MailServer SSL issued..,10')
    ...
    def install_postfix_dovecot(self):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Postfix/dovecot reinstalled.,40')

        except BaseException as msg:
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '%s [install_postfix_dovecot][404]' % (str(msg)), 10)
            return 0
    ...
    def setup_postfix_dovecot_config(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu:
                self.centos_lib_dir_to_ubuntu("/usr/local/EagleEP/install/email-configs-one/master.cf", "/usr/libexec/", "/usr/lib/")
                self.centos_lib_dir_to_ubuntu("/usr/local/EagleEP/install/email-configs-one/main.cf", "/usr/libexec/postfix",
            ...
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_domains.cf", "/etc/postfix/mysql-virtual_domains.cf")
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_forwardings.cf",
                        "/etc/postfix/mysql-virtual_forwardings.cf")
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_mailboxes.cf", "/etc/postfix/mysql-virtual_mailboxes.cf")
...
"""
            ...
            logging.EagleEPLogFileWriter.writeToFile(f'Error in configureOpenDKIM {str(msg)}')
            return 0
    ...
    def ResetEmailConfigurations(self):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing postfix/dovecot..,5')
            ...
            if self.setup_postfix_dovecot_config() == 0:
                logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'setup_postfix_dovecot_config failed. [404].')
                return 0

            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Restoreing OpenDKIM configurations..,70')
            ...
            if self.configureOpenDKIM() == 0:
                logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'configureOpenDKIM failed. [404].')
                return 0


            if self.MailSSL:
                logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Setting up Mail Server SSL if any..,75')
                from plogical.virtualHostUtilities import virtualHostUtilities
    ...
    def EmailLimits(self):
        ...
        try:
            ...
            if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
                ...
                url = "https://platform.cyberpersons.com/EaglepanelAdOns/Adonpermission"
                data = {
                    "name": "all",
                ...
                if (Status == 1):
                    ...
                    return redirect("https://eaglepanel.net/eaglepanel-addons")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='EaglePanel')
