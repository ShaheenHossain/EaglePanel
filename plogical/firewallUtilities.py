...
class FirewallUtilities:
    ...
    def doCommand(command):
        try:
            ...
            if res == 0:
                logging.EagleEPLogFileWriter.writeToFile("Failed to apply rule: " + command + " Error #" + str(res))
                return 0

        except OSError as msg:
            logging.EagleEPLogFileWriter.writeToFile("Failed to apply rule: " + command + " Error: " + str(msg))
            return 0
        except ValueError as msg:
            logging.EagleEPLogFileWriter.writeToFile("Failed to apply rule: " + command + " Error: " + str(msg), 1)
            return 0
        return 1
    ...
    def addSSHKey(tempPath, path=None):
        try:
            ...
            if os.path.exists(pathToSSH):
                ...
                sshFile.writelines("#Created by EaglePanel\n")
                sshFile.close()
