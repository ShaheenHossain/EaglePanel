...
class remoteTransferUtilities:
    ...
    def writeAuthKey(pathToKey):
        try:
            ...
            if presenseCheck == 0:
                ...
                writeToFile.writelines("#Added by EaglePanel\n")
            ...
            logging.EagleEPLogFileWriter.writeToFile("For remote transfer, I am not able to write key to auth file, Error Message: "+str(msg))
    ...
    def remoteTransfer(ipAddress, dir, accountsToTransfer):
        try:
            ...
            if backupUtil.backupUtilities.checkIfHostIsUp(ipAddress) == 1:
                ...
                if checkConn[0] == 0:
                    writeToFile.writelines("[" + time.strftime(
                        "%m.%d.%Y_%H-%M-%S") + "]" + " Connection to:" + ipAddress + " Failed, please resetup this destination from EaglePanel, aborting. [5010]" + "\n")
                    writeToFile.close()
                    return
                else:
                    pass
            ...
            writeToFile.close()
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [remoteTransfer]")
            return [0, str(msg)]
            ...
            try:
                ...
                for virtualHost in accountsToTransfer:
                    try:
                        ...
                        logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [remoteTransferUtilities.backupProcess:173]")
                        pass

                portpath = "/home/eaglepanel/remote_port"
                os.remove(portpath)
                ...
                writeToFile.close()
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [backupProcess]")
    ...
    def sendBackup(completedPathToSend, IPAddress, folderNumber,writeToFile):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile("habbi--------open file:%s"%portpath)

            # Read the contents of the file
            with open(portpath, 'r') as file:
                sshPort = file.readline().strip()

            command = "sudo scp -o StrictHostKeyChecking=no -i /root/.ssh/eaglepanel -P "+ sshPort + " " + completedPathToSend + " root@" + IPAddress + ":/home/backup/transfer-" + folderNumber + "/"
            return_Code = subprocess.call(shlex.split(command), stdout=writeToFile)
            if return_Code == 0:
                logging.EagleEPLogFileWriter.writeToFile("This backup file is run")
            else:
                logging.EagleEPLogFileWriter.writeToFile("This back file is not run")
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [startBackup]")
    ...
    def remoteBackupRestore(backupDir, dir):
        try:
            ...
            writeToFile.writelines(str(msg) + " [5010]" + "\n")
            writeToFile.close()
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [remoteRestore]")
            return [0, msg]
    ...
    def startRestore(backupDir, backupLogPath, dir):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [remoteTransferUtilities.startRestore]")
