...
class IncScheduler(multi.Thread):
    logPath = '/home/eaglepanel/incbackuplogs'
    gitFolder = '/home/eaglepanel/git'
            ...
            for website in os.listdir(IncScheduler.gitFolder):
                ...
                for file in os.listdir(finalPathInside):
                    ...
                    try:
                        ...
                        data['commitMessage'] = 'Auto commit by EaglePanel %s cron on %s' % (
    ...
    def checkDiskUsage():
        ...
        try:
            ...
            message = '%s - Disk Usage Warning - EaglePanel' % (ACLManager.fetchIP())
...
""" % (sender, ", ".join(TO), SUBJECT, domain, currentTime)
                ...
                if jobConfig[IncScheduler.frequency] == type:
                    ...
                    #     'port'] + " -i /root/.ssh/eaglepanel " + destinationConfig['username'] + "@" + \
                    #           destinationConfig[
...
""" % (sender, ", ".join(TO), SUBJECT, domain, currentTime)
                            ...
                            command = "scp -o StrictHostKeyChecking=no -P " + destinationConfig[
                                'port'] + " -i /root/.ssh/eaglepanel " + backupPath + " " + destinationConfig[
    ...
    def forceRunAWSBackup(planName):
        try:
            ...
            for items in plan.websitesinplan_set.all():
                ...
                extraArgs['path'] = '/home/eaglepanel/backups/%s/backup-' % (
    ...
    def v2Backups(function):
        try:
            ...
            import json
            import time
            if os.path.exists('/home/eaglepanel/v2backups'):
                for website in Websites.objects.all():
                    finalConfigPath = f'/home/eaglepanel/v2backups/{website.domain}'
...
""" % (sender, ", ".join(TO), SUBJECT, website.domain, time.strftime("%m.%d.%Y_%H-%M-%S"))
    ...
    def CheckHostName():
        try:
            ...
            virtualHostUtilities.OnBoardingHostName(CurrentHostName, '/home/eaglepanel/onboarding_temp_path', skipRDNSCheck)
        except BaseException as msg:
