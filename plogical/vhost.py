...
class vhost:
    ...
    def addUser(virtualHostUser, path):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(f"{str(msg)} [addingUsers]")
    ...
    def createDirectories(path, virtualHostUser, pathHTML, pathLogs, confPath, completePathToConfigFile):
        try:
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [createDirectories]]")
                #return [0, "[45 Not able to directories for virtual host [createDirectories]]"]
    ...
    def finalizeVhostCreation(virtualHostName, virtualHostUser):
        try:
            ...
            shutil.copy("/usr/local/EagleEP/index.html", f"/home/{virtualHostName}/public_html/index.html")
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [finalizeVhostCreation]")
    ...
    def createConfigInMainVirtualHostFile(virtualHostName):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + "223 [IO Error with main config file [createConfigInMainVirtualHostFile]]")
    ...
    def deleteVirtualHostConfigurations(virtualHostName):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                gitPath = '/home/eaglepanel/git/%s' % (virtualHostName)
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [Not able to remove virtual host configuration from main configuration file.]")
                return 0
            return 1
        else:
            try:
            ...
            try:
                if ApacheVhost.changePHP(phpVersion, vhFile) == 0:
                    ...
                    if not os.path.exists("/usr/local/lsws/lsphp" + str(php) + "/bin/lsphp"):
                        print(0, 'This PHP version is not available on your EaglePanel.')
                        return [0, "[This PHP version is not available on your EaglePanel. [changePHP]"]
                    ...
                    logging.EagleEPLogFileWriter.writeToFile('apache vhost 1')
    ...
    def addRewriteRules(virtualHostName, fileName=None):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [IO Error with per host config file [changePHP]]")
            return 0
    ...
    def findDomainBW(domainName, totalAllowed):
        try:
            ...
            bwmeta = "/home/eaglepanel/%s.bwmeta" % (domainName)
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [findDomainBW]")
            print("0,0")
            return 0, 0
    ...
    def checkIfSSLAliasExists(data, aliasDomain):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [checkIfSSLAliasExists]")
            return 1
    ...
    def createAliasSSLMap(confPath, masterDomain, aliasDomain):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [createAliasSSLMap]")
    ...
    def finalizeDomainCreation(virtualHostUser, path):
        try:
            ...
            FullPath = '%s/%s' % ('/usr/local/EagleEP/tmp', RanddomFileName)

            FNULL = open(os.devnull, 'w')

            #shutil.copy("/usr/local/EagleEP/index.html", path + "/index.html")

            shutil.copy("/usr/local/EagleEP/index.html", FullPath)
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [finalizeDomainCreation]")
    ...
    def createDirectoryForDomain(masterDomain, domain, phpVersion, path, administratorEmail, virtualHostUser,
        ...
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [createDirectoryForDomain]]")
