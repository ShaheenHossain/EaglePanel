...
class ApplicationInstaller(multi.Thread):
    ...
    def UpgradeCP(self):
        command = f'/usr/local/EaglePanel/bin/python /usr/local/EagleEP/plogical/upgrade.py "SoftUpgrade,{self.data["branchSelect"]}"'
    ...
    def installJoomla(self):
        ...
        try:
            ...
            try:
                os.remove('/usr/local/EagleEP/joomla.zip')
                os.remove('/usr/local/EagleEP/lscache_plugin.zip')
                os.remove('/usr/local/EagleEP/pkg_lscache.xml')
                os.remove('/usr/local/EagleEP/pkg_script.php')
            except:
                pass
            ...
            command = 'wget https://raw.githubusercontent.com/litespeedtech/lscache-joomla/master/package/lscache-1.3.1.zip -O /usr/local/EagleEP/joomla.zip'
            ProcessUtilities.executioner(command)

            command = 'unzip -o /usr/local/EagleEP/joomla.zip -d /usr/local/EagleEP/'
            ProcessUtilities.executioner(command)

            command = '%s extension:installfile %s --www %s /usr/local/EagleEP/lscache_plugin.zip' % (
            joomlaPath, dbUser, finalPath)
            ProcessUtilities.executioner(command)

            command = '%s extension:installfile %s --www %s /usr/local/EagleEP/com_lscache.zip' % (
            joomlaPath, dbUser, finalPath)
            ProcessUtilities.executioner(command)
    ...
    def DeployWordPress(self):
        try:
            ...
            self.extraArgs['tempStatusPath'] = "/home/eaglepanel/" + str(randint(1000, 9999))
            ...
            if cronData.find('WPAutoUpdates.py') == -1:
                ...
                writeToFile.write(
                    '0 12 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/WPAutoUpdates.py\n')
                writeToFile.close()
    ...
    def CreateStagingNow(self):
        try:
            ...
            tempPath = '%s/%s' % ('/usr/local/EagleEP/tmp', str(randint(1000, 9999)))
            ...
            if os.path.exists('/usr/local/EagleEP/debug'):
                logging.writeToFile("Creating website result....%s" % coreResult1)
    ...
    def DeploytoProduction(self):
        ...
        try:
            ...
            ### Create secure folder
            ACLManager.CreateSecureDir()
            self.tempPath = '%s/%s' % ('/usr/local/EagleEP/tmp', str(randint(1000, 9999)))
    ...
    def WPCreateBackup(self):
        try:
            ...
            if Backuptype == "1":
                ...
                self.tempPath = '%s/%s' % ('/usr/local/EagleEP/tmp', RandomPath)
                ...
                # command = "sudo -u %s touch /home/eaglepanel/config.json" % (VHuser)
                # ProcessUtilities.executioner(command)
                ###### write into config

                json_object = json.dumps(config, indent=4)
                configPath = "/home/eaglepanel/" + str(randint(1000, 9999))
        ...
        try:
            ...
            if BackupDestination == 'SFTP':
                ...
                with pysftp.Connection(HostName, username=Username, password=Password, cnopts=cnopts) as sftp:
                    ...
                    loaclpath = "/home/eaglepanel/%s.tar.gz" % BackUpFileName
                    ...
                    #         unzippathdb = "%s/ab/usr/local/EagleEP/tmp/%s/%s" % (
                    ...
                    #                     raise BaseException(stdout)
                    #
                    #                 unzippath = "%s/ab/usr/local/EagleEP/tmp/%s/public_html/" % (
                    ...
                    #         if oldhome == "0":
                    #             unzippath = "%s/ab/usr/local/EagleEP/tmp/%s/public_html/" % (self.tempPath, oldtemppath)
                    #         else:
                    #             unzippath = "%s/ab/usr/local/EagleEP/tmp/%s/public_html/public_html/" % (
                    #                 self.tempPath, oldtemppath)
                    #
                    #         command = "sudo -u %s cp -R %s* %s" % (VHuser, unzippath, newWPpath)
                    #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
                    ...
                    #
                    #                 # dump Mysql file in unzippath path
                    #                 unzippath2 = "%s/ab/usr/local/EagleEP/tmp/%s/%s" % (
                    #                     self.tempPath, oldtemppath, DumpFileName)
            ...
            ###S#Backups
                ...
                FinalZipPath = "/home/eaglepanel/%s.tar.gz" % (uploadfilename)
                try:
                ...
                #
                #         # dump Mysql file in unzippath path
                #         unzippathdb = "%s/ab/usr/local/EagleEP/tmp/%s/%s" % (self.tempPath, oldtemppath, DumpFileName)
                #         # command = "mysql -u root %s < %s" % (Finaldbname, unzippathdb)
                #         command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp --allow-root --skip-plugins --skip-themes --path=%s --quiet db import %s' % (
                #             VHuser, FinalPHPPath, newWPpath, unzippathdb)
                #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
                ...
                #
                #         # dump Mysql file in unzippath path
                #         unzippath2 = "%s/ab/usr/local/EagleEP/tmp/%s/%s" % (self.tempPath, oldtemppath, DumpFileName)
                #         # command = "mysql -u root %s < %s" % (Finaldbname, unzippath2)
                #         command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp --allow-root --skip-plugins --skip-themes --path=%s --quiet db import %s' % (
                #             VHuser, FinalPHPPath, newWPpath, unzippath2)
                #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
    ...
    def StartOCRestore(self):
        try:
            ...
            sftp.get(f'cpbackups/{folder}/{backupfile}', f'/home/eaglepanel/{backupfile}', callback=self.UpdateDownloadStatus)
            ...
            command = f'mv /home/eaglepanel/{backupfile} /home/backup/{backupfile}'
            ProcessUtilities.executioner(command)
...
def main():
    parser = argparse.ArgumentParser(description='EaglePanel Application Installer')
