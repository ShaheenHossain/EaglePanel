...
class backupSchedule:
    ...
    ERROR = 1
    backupLog = ''
    runningPath = '/home/eaglepanel/remoteBackupPID'
    ...
    def createLocalBackup(virtualHost, backupLogPath):
        try:
            ...
            pathToFile = "/home/eaglepanel/" + str(randint(1000, 9999))
            ...
            time.sleep(5)
            schedulerPath = '/home/eaglepanel/%s-backup.txt' % (virtualHost)
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [119:startBackup]")
            backupSchedule.remoteBackupLogging(backupLogPath,
                                               "Local backup creating failed for %s, Error message: %s" % (
    ...
    def createBackup(virtualHost, ipAddress, backupLogPath , port='22', user='root'):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [backupSchedule.createBackup]")
    ...
    def sendBackup(backupPath, IPAddress, backupLogPath , port='22', user='root'):
        try:
            ...
            command = "scp -o StrictHostKeyChecking=no -P "+port+" -i /root/.ssh/eaglepanel " + backupPath + " " + user + "@" + IPAddress+":~/backup/" + ipAddressLocal + "/" + time.strftime("%m.%d.%Y_%H-%M-%S") + "/"
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [189:startBackup]")
    ...
    def prepare():
        try:
            ...
            if os.path.exists(backupSchedule.runningPath):
                ...
                if output.find('/usr/local/EagleEP/plogical/backupSchedule.py') > -1 and output.find(pid) > -1:
                    print(
            ...
            if checkConn[0] == 0:
                backupSchedule.remoteBackupLogging(backupLogPath,
                                                   "Connection to: " + ipAddress + " Failed, please resetup this destination from EaglePanel, aborting.")
                return 0
            else:
                ## Create backup dir on remote server in ~/backup

                command = "ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/eaglepanel " + user + "@" + ipAddress + " mkdir -p ~/backup/" + ipAddressLocal + "/" + time.strftime(
                    "%a-%b")
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [prepare]")
...
def handler(signum, frame):
    ...
    logging.EagleEPLogFileWriter.writeToFile('Signal: %s, time spent: %s' % (str(signum), str(diff.total_seconds())))
