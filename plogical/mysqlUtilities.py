...
class mysqlUtilities:
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile('Local IP for MySQL: %s' % (mysqlUtilities.LOCALHOST))
    ...
    def createDatabase(dbname,dbuser,dbpassword, dbcreate = 1, host = None):
        try:
            ...
            if dbcreate:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile(query)
            ...
            if mysqlUtilities.RDS == 0:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile("GRANT ALL PRIVILEGES ON " + dbname + ".* TO '" + dbuser + "'@'%s'" % (HostToUse))
            else:
                cursor.execute(
                    "GRANT INDEX, DROP, UPDATE, ALTER, CREATE, SELECT, INSERT, DELETE ON " + dbname + ".* TO '" + dbuser + "'@'%s'" % (HostToUse))
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile("GRANT INDEX, DROP, UPDATE, ALTER, CREATE, SELECT, INSERT, DELETE ON " + dbname + ".* TO '" + dbuser + "'@'%s'" % (HostToUse))
            ...
            if dbcreate:
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile('Deleting database because failed to create %s' % (dbname))
                #mysqlUtilities.deleteDatabase(dbname, dbuser)
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[createDatabase]")
            return 0
    ...
    def createDBUser(dbuser, dbpassword):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[createDBUser]")
            return 0
    ...
    def deleteDatabase(dbname, dbuser):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[deleteDatabase]")
            return str(msg)
    ...
    def createDatabaseBackup(databaseName, tempStoragePath, rustic=0, RusticRepoName = None, externalApp = None):
        try:
            ...
            cnfPath = '/home/eaglepanel/.my.cnf'
...
""" % (password, password)
            ...
            if rustic == 0:
                ...
                command = 'mysqldump --defaults-extra-file=/home/eaglepanel/.my.cnf -u %s --host=%s --port %s %s' % (mysqluser, mysqlhost, mysqlport, databaseName)
                ...
                try:
                    errorPath = '/home/eaglepanel/error-logs.txt'
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[createDatabaseBackup]")
            return 0
...
""" % (password, password)
                ...
                os.chmod(cnfPath, 0o600)
                command = 'chown eaglepanel:eaglepanel %s' % (cnfPath)
                subprocess.call(shlex.split(command))

            if rustic == 0:

                command = 'mysql --defaults-extra-file=/home/eaglepanel/.my.cnf -u %s --host=%s --port %s %s' % (mysqluser, mysqlhost, mysqlport, databaseName)
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile(f'{command} {tempStoragePath}/{databaseName} ' )
                cmd = shlex.split(command)
                ...
                if additionalName == None:
                    ...
                    if res != 0:
                        logging.EagleEPLogFileWriter.writeToFile("Could not restore MYSQL database: " + databaseName +"! [restoreDatabaseBackup]")
                        return 0
                else:
                    ...
                    if res != 0:
                        logging.EagleEPLogFileWriter.writeToFile("Could not restore MYSQL database: " + additionalName + "! [restoreDatabaseBackup]")
                        return 0
                ...
                command = f'sudo -u {externalApp} rustic -r {RusticRepoName} dump {snapshotid}:{databaseName}.sql --password "" 2>/dev/null | mysql --defaults-extra-file=/home/eaglepanel/.my.cnf -u %s --host=%s --port %s %s' % (
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[restoreDatabaseBackup]")
            return 0
    ...
    def submitDBDeletion(dbName):
        try:
            ...
            if result == 1:
                ...
                databaseToBeDeleted.delete()
                logging.EagleEPLogFileWriter.writeToFile('Deleted database with some errors. Error: %s' % (result))
                return 1,'None'
    ...
    def showStatus():
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[showStatus]")
            return 0
    ...
    def fetchVariables():
        try:
            ...
            for items in result:
                logging.EagleEPLogFileWriter.writeToFile(str(items))
    ...
    def fetchDatabases():
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[fetchDatabases]")
            return 0
    ...
    def fetchTableData(name):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[fetchTableData]")
            return 0
    ...
    def changePassword(userName, dbPassword, encrypt = None, host = None):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.changePassword]")
            return 0
    ...
    def fetchuser(databaseName):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.fetchuser]")
            return 0
    ...
    def allowRemoteAccess(dbName, userName, remoteIP):
        try:
            ...
            execPath = "/usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/mysqlUtilities.py enableRemoteMYSQL"
            ProcessUtilities.executioner(execPath)

            return 1

        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.allowRemoteAccess]")
            return 0
...
''' % (ipAddressLocal)
        ...
        try:
            ...
            if createUser:
                try:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile('%s [addUserToDB:937]' % (str(msg)))
                    try:
                        cursor.execute("DROP USER '%s'@'%s'" % (user, mysqlUtilities.LOCALHOST))
                        cursor.execute(
                            "CREATE USER '" + user + "'@'%s' IDENTIFIED BY '" % (mysqlUtilities.LOCALHOST) + password + "'")
                    except BaseException as msg:
                        logging.EagleEPLogFileWriter.writeToFile('%s [addUserToDB:943]'  % (str(msg)))
            ...
            if mysqlUtilities.RDS == 0:
                ...
                try:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile('%s [addUserToDB:953]' % (str(msg)))
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[addUserToDB]")
            return 0
    ...
    def UpgradeMariaDB(versionToInstall, tempStatusPath):
        ...
        if result.find(versionToInstall) > -1:
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, f'MySQL is already {result}. [200]')
            return 0

        logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Creating backup of MySQL..,10')

        MySQLBackupDir = '/var/lib/mysql-backupcp'

        from os import getuid
        if getuid() != 0:
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'This function should run as root. [404]')
        ...
        if not os.path.exists(MySQLBackupDir):
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'MySQL backup created..,20')
