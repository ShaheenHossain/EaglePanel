...
class sslUtilities:
        ...
        try:
            ...
            with open(cert_path, 'rb') as cert_file:
                ...
                if san_extension:
                    ...
                    try:
                        logging.EagleEPLogFileWriter.writeToFile(f'Covered domains: {str(san_domains)}')
                    except:
                        pass
    ...
    def checkIfSSLMap(virtualHostName):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkIfSSLMap]]")
            return 0
    ...
    def checkSSLListener():
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkSSLListener]]")
            return str(msg)
        return 0
    ...
    def checkSSLIPv6Listener():
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkSSLIPv6Listener]]")
            return str(msg)
        return 0
...
'''
    ...
    def installSSLForDomain(virtualHostName, adminEmail='example@example.org'):
        ...
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile('%s [installSSLForDomain:72]' % (str(msg)))
        ...
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [installSSLForDomain]]")
                return 0
        else:
    ...
    """
                ...
                logging.EagleEPLogFileWriter.writeToFile('hello world aaa')
    ...
    def obtainSSLForADomain(virtualHostName, adminEmail, sslpath, aliasDomain=None):
        ...
        try:
            ...
            if resp.status_code == 200:
                logging.EagleEPLogFileWriter.writeToFile(f'Status Code: 200 for: {URLFetchPathWWW}')
                WWWStatus = 1
            else:
                logging.EagleEPLogFileWriter.writeToFile(
        ...
        try:
            ...
            if resp.status_code == 200:
                logging.EagleEPLogFileWriter.writeToFile(f'Status Code: 200 for: {URLFetchPathNONWWW}')
                NONWWWStatus = 1
            else:
                logging.EagleEPLogFileWriter.writeToFile(f'Status Code: {str(resp.status_code)} for: {URLFetchPathNONWWW}. Error: {resp.text}')
        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile(
        ...
        try:
            ...
            #     acmePath = '/home/eaglepanel/.acme.sh/acme.sh'
            ...
            if aliasDomain is None:
                ...
                try:
                    ...
                    #ResultText = open(logging.EagleEPLogFileWriter.fileName, 'r').read()
                    #CurrentMessage = "Trying to obtain SSL for: " + virtualHostName + " and: www." + virtualHostName
                    if (WWWStatus and NONWWWStatus):

                        #logging.EagleEPLogFileWriter.writeToFile(CurrentMessage, 0)

                        logging.EagleEPLogFileWriter.writeToFile(command, 0)

                        output = subprocess.check_output(shlex.split(command)).decode("utf-8")
                        logging.EagleEPLogFileWriter.writeToFile("Successfully obtained SSL for: " + virtualHostName + " and: www." + virtualHostName, 0)

                        logging.EagleEPLogFileWriter.SendEmail(sender_email, adminEmail, output, 'SSL Notification for %s.' % (virtualHostName))
                    else:
                        logging.EagleEPLogFileWriter.writeToFile(command, 0)
                    ...
                    try:
                        ...
                        if NONWWWStatus:
                            finalText = '%s\nTrying to obtain SSL for: %s' % (finalText, virtualHostName)
                            logging.EagleEPLogFileWriter.writeToFile("Trying to obtain SSL for: " + virtualHostName, 0)
                            logging.EagleEPLogFileWriter.writeToFile(command)
                            output = subprocess.check_output(shlex.split(command)).decode("utf-8")
                            logging.EagleEPLogFileWriter.writeToFile(
                                "Successfully obtained SSL for: " + virtualHostName, 0)
                            finalText = '%s\nSuccessfully obtained SSL for: %s.' % (finalText, virtualHostName)
                            logging.EagleEPLogFileWriter.SendEmail(sender_email, adminEmail, finalText,
                        ...
                        logging.EagleEPLogFileWriter.writeToFile('Failed to obtain SSL, issuing self-signed SSL for: ' + virtualHostName, 0)
                        logging.EagleEPLogFileWriter.SendEmail(sender_email, adminEmail, 'Failed to obtain SSL, issuing self-signed SSL for: ' + virtualHostName,
                                                               'SSL Notification for %s.' % (virtualHostName))
                        return 0
            else:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [Failed to obtain SSL. [obtainSSLForADomain]]")
            return 0
...
def issueSSLForDomain(domain, adminEmail, sslpath, aliasDomain=None):
    try:
        if sslUtilities.obtainSSLForADomain(domain, adminEmail, sslpath, aliasDomain) == 1:
            ...
            #### if in any case ssl failed to obtain and EaglePanel try to issue self-signed ssl, first check if ssl already present.
            ...
            if os.path.exists(pathToStoreSSLFullChain):
                ...
                if SSLProvider != 'Denial':
                    if sslUtilities.installSSLForDomain(domain) == 1:
                        logging.EagleEPLogFileWriter.writeToFile("We are not able to get new SSL for " + domain + ". But there is an existing SSL, it might only be for the main domain (excluding www).")
            ...
            if sslUtilities.installSSLForDomain(domain) == 1:
                logging.EagleEPLogFileWriter.writeToFile("Self signed SSL issued for " + domain + ".")
