...
class remoteBackup:
    ...
    def getKey(ipAddress, password):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [getKey]")
            ...
            for backup in os.listdir(backupDir):
                ...
                if backup.endswith(ext):
                    ...
                    if data['restoreStatus'] == 1:
                        ...
                        logging.EagleEPLogFileWriter.writeToFile("Could not start restore process for: " + backup)
                        writeToFile = open(backupLogPath, "a")
                        writeToFile.writelines("\n")
                        writeToFile.writelines("\n")
                        writeToFile.writelines("[" + time.strftime(
    ...
    def postRemoteTransfer(ipAddress, ownIP ,password, sshkey):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [postRemoteTransfer]")
            return [0, msg]
    ...
    def createBackup(virtualHost, ipAddress,writeToFile, dir):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [214:startBackup]")
    ...
    def sendBackup(completedPathToSend, IPAddress, folderNumber,writeToFile):
        try:
            ...
            command = 'sudo rsync -avz -e "ssh  -i /root/.ssh/eaglepanel -o StrictHostKeyChecking=no" ' + completedPathToSend + ' root@' + IPAddress + ':/home/backup/transfer-'+folderNumber
    ...
    def remoteTransfer(ipAddress, dir,accountsToTransfer):
        try:
            ...
            if backupUtil.backupUtilities.checkIfHostIsUp(ipAddress) == 1:
                ...
                if checkConn[0] == 0:
                    writeToFile.writelines("[" + time.strftime(
                        "%m.%d.%Y_%H-%M-%S") + "]" + " Connection to:" + ipAddress + " Failed, please resetup this destination from EaglePanel, aborting." + "\n")
                    writeToFile.close()
