...
class CPBackupsV2(multi.Thread):
    ...
    def __init__(self, data):
        ...
        try:
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(f"Refresh Token: {refreshToken}")
                ...
                logging.EagleEPLogFileWriter.writeToFile(f"Token Not upadate inside. Error: {str(msg)}")
                pass
            ...
            # else:
            #     logging.EagleEPLogFileWriter.writeToFile("Token Not upadate..........")
        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile("Error update token............%s"%msg)
        ...
        self.StatusFile = f'/home/eaglepanel/{self.website.domain}_rustic_backup_log'
        self.StatusFile_Restore = f'/home/eaglepanel/{self.website.domain}_rustic_backup_log_Restore'
    ...
    def run(self):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + ' [CPBackupsV2.run]')

    def FetchSnapShots(self):
        try:
            command = f'rustic -r {self.repo} snapshots --password "" --json 2>/dev/null'
            # SLSkjoSCczb6wxTMCBPmBMGq/UDSpp28-u cyber5986 rustic -r rclone:None:eaglepanel.net snapshots --password "" --json 2>/dev/null
            result = json.loads(
...
'''
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + ' [Configure.run]')
    ...
    def BackupConfig(self):
        ...
        # command = f'chown eaglepanel:eaglepanel {self.FinalPathRuctic}'
        # ProcessUtilities.executioner(command)
        ...
        command = f'chown eaglepanel:eaglepanel {self.FinalPathRuctic}/config.json'
        ProcessUtilities.executioner(command)
    ...
    def InitiateBackup(self):
        logging.EagleEPLogFileWriter.writeToFile("[Create Backup start]")
        ...
        while (1):
            ...
            if self.website.BackupLock == 0:
                ...
                # command = f'chown eaglepanel:eaglepanel {self.FinalPath}'
                # ProcessUtilities.executioner(command)
    ...
    def RestoreConfig(self):
        try:
            ...
            # try:
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.EagleEPLogFileWriter.writeToFile(f'ACL in config: {ConfigContent["acl"]}')
            #     acl = ACL.objects.get(name=ConfigContent['acl']['name'])
            # except:
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.EagleEPLogFileWriter.writeToFile('ACL Already existed.')
            ...
            #
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.EagleEPLogFileWriter.writeToFile(f'Passed content to Create ACL Func: {json.dumps(dataToPass)}')
            #
            #     resp = createACLFunc(requestNew)
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.EagleEPLogFileWriter.writeToFile(f'CreateACLFunc stats: {str(resp.content)}')
            ...
            try:
                ...
                for database in ConfigContent['databases']:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.EagleEPLogFileWriter.writeToFile(f'Databasename: {dbName}')
                    ...
                    for databaseUser in databaseUsers:
                        ...
                        if first:
                            ...
                            try:
                                ...
                                logging.EagleEPLogFileWriter.writeToFile('Database exists, changing Database password.. %s' % (dbName))

                                if mysqlUtilities.mysqlUtilities.changePassword(dbUser, password, 1, dbHost) == 0:
                                    logging.EagleEPLogFileWriter.writeToFile('Failed changing password for database: %s' % (dbName))
                                else:
                                    logging.EagleEPLogFileWriter.writeToFile('Password successfully changed for database: %s.' % (dbName))

                            except:

                                logging.EagleEPLogFileWriter.writeToFile('Database did not exist, creating new.. %s' % (dbName))

                                if mysqlUtilities.mysqlUtilities.createDatabase(dbName, dbUser, "eaglepanel") == 0:
                                    logging.EagleEPLogFileWriter.writeToFile('Failed the creation of database: %s' % (dbName))
                                else:
                                    logging.EagleEPLogFileWriter.writeToFile('Database: %s successfully created.' % (dbName))
    ...
    def FetchCurrentSchedules(website):
        try:
            finalConfigPath = f'/home/eaglepanel/v2backups/{website}'
    ...
    def refresh_V2Gdive_token(refresh_token):
        try:
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile('Current Token: ' + refresh_token )
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile('newtoken: ' + newtoken )
                logging.EagleEPLogFileWriter.writeToFile(r.text)

            return newtoken
        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile(f'Error in tkupdate: {str(msg)}')
    ...
    def CreateScheduleV2(website, repo, frequency, websiteData, websiteDatabases, websiteEmails, retention):
        try:
            ...
            if not os.path.exists('/home/eaglepanel/v2backups/'):

                command = 'mkdir -p /home/eaglepanel/v2backups/'
...
if __name__ == "__main__":
    try:
        ...
        cpbuv2 = CPBackupsV2(
            {'function': 'InitiateRestore', 'domain': 'eaglepanel.net', 'BasePath': '/home/backup', 'SnapShotID': 1,
             'BackendName': 'usman'})
