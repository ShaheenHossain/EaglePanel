...
class cPanelImporter:
    ...
    def SetupSSL(self, path, domain):
        ...
        KeyPath = '/home/eaglepanel/%s' % (str(randint(1000, 9999)))
        ...
        CertPath = '/home/eaglepanel/%s' % (str(randint(1000, 9999)))
    ...
    def CreateMainWebsite(self):
        try:
            ...
            if os.path.exists(SSLPath):
                ...
                message = 'SSL not detected for %s, you can later issue SSL from Manage SSL in EaglePanel.' % (DomainName)
                logging.statusWriter(self.logFile, message, 1)
                ...
                try:
                    ...
                    if os.path.exists(SSLPath):
                        ...
                        if os.path.exists(SSLPath):
                            ...
                            message = 'SSL not detected for %s, you can later issue SSL from Manage SSL in EaglePanel.' % (
                                items)
                            logging.statusWriter(self.logFile, message, 1)
    ...
    def CreateDNSRecords(self):
        try:
            ...
            message = 'We are going to create DNS records now, please note we will not create DKIM records. Make sure to create them from EaglePanel interface using our DKIM manager.'
            logging.statusWriter(self.logFile, message, 1)
    ...
    def RestoreDatabases(self):
        try:
            ...
            for items in data:
                if items.find("--") > -1 or items.find("'eaglepanel'@") > -1:
                    continue
                try:
                    ...
                    if items.find('.') > -1:
                        for it in os.listdir(FinalMailDomainPath):
                            try:
                                ...
                                mailUtilities.createEmailAccount(items, it, 'eaglepanel')
...
def main():
    LogFile = '/home/eaglepanel/%s' % (str(randint(1000, 9999)))
    message = 'Backup logs to be generated in %s' % (LogFile)
    print(message)

    parser = argparse.ArgumentParser(description='EaglePanel cPanel Importer')
