...
class backupUtilities:
    ...
    completeKeyPath = "/home/eaglepanel/.ssh"
    destinationsPath = "/home/eaglepanel/destinations"
    licenseKey = '/usr/local/lsws/conf/license.key'
    NiceDefault = '10'
    CPUDefault = '1000'
    CloudBackupConfigPath = '/home/eaglepanel/CloudBackup.json'
    time = 10
    ...
    def prepareBackupMeta(backupDomain, backupName, tempStoragePath, backupPath, FromInner=1):
        try:
            ...
            if FromInner:
                ...
                #logging.EagleEPLogFileWriter.statusWriter(status, 'Setting up meta data..')
                command = f"echo 'Setting up meta data..' > {status}"
                ProcessUtilities.executioner(command, website.externalApp)
            else:
                status = '/home/eaglepanel/dummy'

            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Creating meta for {backupDomain}.')
            ...
            ### try to take care of - https://github.com/usmannasir/eaglepanel/issues/1196
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile('%s. [167:prepMeta]' % (str(msg)))
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile('%s. [158:prepMeta]' % (str(msg)))
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile('%s. [179:prepMeta]' % (str(msg)))
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile(f'Path to meta file {metaPath}')
            ...
            logging.EagleEPLogFileWriter.writeToFile(f"{str(msg)} [207][5009]")
            if FromInner:
                #logging.EagleEPLogFileWriter.statusWriter(status, "%s [207][5009]" % (str(msg)), status)
    ...
    def startBackup(tempStoragePath, backupName, backupPath, metaPath=None):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(backupFileNamePath, backupName)

            #####

            status = os.path.join(backupPath, 'status')

            logging.EagleEPLogFileWriter.statusWriter(status, "Making archive of home directory.\n")
            ...
            ### doing backup of whole dir and keeping it in public_html folder will restore from here - ref https://github.com/usmannasir/eaglepanel/issues/1196
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Backing up databases..")
            print('1,None')
            ...
            status = os.path.join(backupPath, 'status')
            logging.EagleEPLogFileWriter.statusWriter(status, "Aborted, " + str(msg) + ".[365] [5009]")
    ...
    def BackupRoot(tempStoragePath, backupName, backupPath, metaPath=None, externalApp = None, CPHomeStorage=None):
        ...
        ### CPHomeStorage /home/eaglepanel/<random_number>
        ...
        if os.path.exists(sslStoragePath):
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(f'{str(msg)}. [283:startBackup]')
        ...
        try:
            for childDomain in childDomains:
                ...
                TempConfPath = f'/home/eaglepanel/{actualChildDomain}.vhost.conf'
        ...
        if os.path.islink(status) or os.path.islink(tempStoragePath or os.path.islink(backupPath)) or os.path.islink(
                metaPath):
            logging.EagleEPLogFileWriter.writeToFile('symlinked.')
            #logging.EagleEPLogFileWriter.statusWriter(status, 'Symlink attack. [365][5009]')
            return 0

        ## backup email accounts

        if externalApp == None:
            logging.EagleEPLogFileWriter.statusWriter(status, "Backing up email accounts..\n")
        else:
        ...
        try:
            ...
            if externalApp == None:
                logging.EagleEPLogFileWriter.statusWriter(status, "Preparing final compressed package..\n")
            else:
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile('%s. [backupRoot:499]' % str(msg))
                for items in backupObs:
                    items.status = 1
                    items.size = totalSize
                    items.save()
            ...
            if externalApp == None:
                logging.EagleEPLogFileWriter.statusWriter(status, "Completed\n")
            else:
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, '%s. [511:BackupRoot][[5009]]\n' % str(msg))
            if externalApp == None:
                logging.EagleEPLogFileWriter.statusWriter(status, '%s. [511:BackupRoot][[5009]]\n')
            else:
    ...
    def initiateBackup(tempStoragePath, backupName, backupPath):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [initiateBackup]")
        ...
        try:
            ...
            for database in databases:
                ...
                if (VERSION == '2.1' or VERSION == '2.3') and int(BUILD) >= 1:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile('Backup version 2.1.1+ detected..')
                    databaseUsers = database.findall('databaseUsers')
                    for databaseUser in databaseUsers:

                        dbUser = databaseUser.find('dbUser').text
                        res = mysqlUtilities.mysqlUtilities.createDatabase(dbName, dbUser, 'eaglepanel')
                        if res == 0:
                    ...
                    if mysqlUtilities.mysqlUtilities.createDatabase(dbName, dbUser, "eaglepanel") == 0:
                        raise BaseException
    ...
    def startRestore(backupName, dir):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(pid, str(os.getpid()))

            status = os.path.join(completPath, 'status')
            logging.EagleEPLogFileWriter.statusWriter(status, "Extracting Main Archive!")
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Creating Accounts,Databases and DNS records!")
            ...
            if result[0] == 1:
                ...
                if os.path.exists(sslStoragePath):
                    ...
                    try:
                        ...
                        logging.EagleEPLogFileWriter.writeToFile('%s. [555:startRestore]' % (str(msg)))

            else:
                logging.EagleEPLogFileWriter.statusWriter(status, "Error Message: " + result[
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Creating Child Domains!")
            ...
            try:
                for childDomain in childDomains:
                    ...
                    if retValues[0] == 1:
                        ...
                        logging.EagleEPLogFileWriter.writeToFile('Error domain %s' % (domain))
                        logging.EagleEPLogFileWriter.statusWriter(status, "Error Message: " + retValues[
                ...
                status.close()
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [startRestore]")
                return 0

            ## Restore Aliases

            logging.EagleEPLogFileWriter.statusWriter(status, "Restoring Domain Aliases!")
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Restoring email accounts!")
            ...
            try:
                ...
                logging.EagleEPLogFileWriter.statusWriter(status, "Error Message: " + str(
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Restoring Databases!")
                ...
                if (VERSION == '2.1' or VERSION == '2.3') and int(BUILD) >= 1:
                    ...
                    for databaseUser in databaseUsers:
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            ...
                            logging.EagleEPLogFileWriter.writeToFile('Database user: %s' % (dbUser))
                            logging.EagleEPLogFileWriter.writeToFile('Database host: %s' % (dbHost))
                            logging.EagleEPLogFileWriter.writeToFile('Database password: %s' % (password))
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Extracting web home data!")
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Extracting email accounts!")
            ...
            logging.EagleEPLogFileWriter.statusWriter(status, "Done")
            ...
            status = os.path.join(completPath, 'status')
            logging.EagleEPLogFileWriter.statusWriter(status, str(msg) + " [736][5009]")
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [startRestore]")
    ...
    def initiateRestore(backupName, dir):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [initiateRestore]")
    ...
    #         ## Temp changes
    #
    #         command = 'chmod 600 %s' % ('/root/.ssh/eaglepanel.pub')
    #         ProcessUtilities.executioner(command)
    #
    #         command = "scp -o StrictHostKeyChecking=no -P " + port + " /root/.ssh/eaglepanel.pub " + user + "@" + IPAddress + ":~/.ssh/authorized_keys"
    #         setupKeys = pexpect.spawn(command, timeout=3)
    #
    #         if os.path.exists(ProcessUtilities.debugPath):
    #             logging.EagleEPLogFileWriter.writeToFile(command)
    #
    ...
    #         ## Temp changes
    #
    #         command = 'chmod 644 %s' % ('/root/.ssh/eaglepanel.pub')
    #         ProcessUtilities.executioner(command)
    #
    #         return [1, "None"]
    #
    ...
    #         ProcessUtilities.executioner(command)
    #
    #         logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [sendKey]")
    ...
    #             pass
    #         else:
    #             logging.EagleEPLogFileWriter.writeToFile("Host is Down.")
    ...
    def sendKey(IPAddress, password, port='22', user='root'):
        try:
            ...
            if os.path.exists('/root/.ssh/eaglepanel.pub'):
                pass
            else:
            ...
            sftp = ssh.open_sftp()
            sftp.put('/root/.ssh/eaglepanel.pub', '.ssh/authorized_keys')
            sftp.close()
    ...
    def setupSSHKeys(IPAddress, password, port='22', user='root'):
        try:
            ...
            if password != 'NOT-NEEDED':
                ...
                private_key_path = '/root/.ssh/eaglepanel'
    ...
    def checkIfHostIsUp(IPAddress):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[checkIfHostIsUp]")
    ...
    def checkConnection(IPAddress, port='22', user='root'):
        try:
            ...
            command = "sudo ssh -i /root/.ssh/eaglepanel -o StrictHostKeyChecking=no -p " + port + ' ' + user + "@" + IPAddress
            ...
            logging.EagleEPLogFileWriter.writeToFile("Timeout " + IPAddress + " [checkConnection]")
            return [0, "371 Timeout while making connection to this server [checkConnection]"]
        except pexpect.EOF as msg:
            logging.EagleEPLogFileWriter.writeToFile("EOF " + IPAddress + "[checkConnection]")
            return [0, "374 Remote Server is not able to authenticate for transfer to initiate. [checkConnection]"]
        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " " + IPAddress + " [checkConnection]")
    ...
    def verifyHostKey(IPAddress, port='22', user='root'):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile("Timeout [verifyHostKey]")
            return [0, "Timeout [verifyHostKey]"]
        except pexpect.EOF as msg:
            logging.EagleEPLogFileWriter.writeToFile("EOF [verifyHostKey]")
            return [0, "EOF [verifyHostKey]"]
        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [verifyHostKey]")
    ...
    def createBackupDir(IPAddress, port='22', user='root'):
        ...
        try:
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/eaglepanel " + user + "@" + IPAddress + " mkdir ~/backup"
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/eaglepanel " + user + "@" + IPAddress + ' "cat ~/.ssh/authorized_keys ~/.ssh/temp > ~/.ssh/authorized_temp"'
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/eaglepanel " + user + "@" + IPAddress + ' "cat ~/.ssh/authorized_temp > ~/.ssh/authorized_keys"'
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [createBackupDir]")
            return 0
    ...
    def host_key_verification(IPAddress):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [host_key_verification]")
            return 0
    ...
    def getAliases(masterDomain):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [getAliases]")
            print(0)
    ...
    def CheckIfSleepNeeded(self):
        import psutil
        while (1):
            logging.EagleEPLogFileWriter.writeToFile('Current CPU percent %s.' % (int(psutil.cpu_percent(interval=None))))
    ...
    def CloudBackups(self):
        ...
        command = 'nice -n %s tar czf %s.tar.gz -C %s %s' % (self.nice, self.BackupPath, '/home/eaglepanel/backups/%s' % (self.extraArgs['domain']), self.BackupPath.split('/')[-1])
        ProcessUtilities.executioner(command)
        ...
        command = 'chown eaglepanel:eaglepanel %s' % (finalPath)
        ProcessUtilities.executioner(command)
        ...
        if self.extraArgs['port'] != 0:
            ...
            command = "scp -o StrictHostKeyChecking=no -P %s -i /root/.ssh/eaglepanel %s root@%s:/home/eaglepanel/backups/%s/" % (self.extraArgs['port'], finalPath, self.extraArgs['ip'], self.extraArgs['destinationDomain'])
    ...
    def SubmitCloudBackupRestore(self):
        try:
            ...
            self.BackupPath = '/home/eaglepanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'])
            self.website = Websites.objects.get(domain=self.extraArgs['domain'])
            ...
            command = 'tar -xf %s -C %s' % (self.BackupPath, '/home/eaglepanel/backups/%s/' % (self.extraArgs['domain']))
            ProcessUtilities.executioner(command)

            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
                                                      'Main Archive extracted,20')

            self.extractedPath = '/home/eaglepanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].rstrip('.tar.gz'))
                    ...
                    for child in childDomains:
                        try:
                            ...
                            virtualHostUtilities.createDomain(self.website.domain, child['domain'], child['php'], child['path'], 1, 0, 0,
                                                              self.website.admin.userName, 0, "/home/eaglepanel/" + str(randint(1000, 9999)))

                except BaseException as msg:
                    logging.EagleEPLogFileWriter.writeToFile('%s [SubmitCloudBackupRestore:1533]' % str(msg))
            ...
            for db in databases:
                ...
                if mysqlUtilities.mysqlUtilities.createDatabase(db['databaseName'], db['databaseUser'], "eaglepanel") == 0:
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '%s [404].' % str(msg))

    ### Cloud Backup functions ends

    def fetchAWSKeys(self):
        path = '/home/eaglepanel/.aws'
    ...
    def SubmitS3BackupRestore(self):
        ...
        try:
            ...
            self.BackupPath = '/home/eaglepanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].split('/')[-1])
            ...
            self.extractedPath = '/home/eaglepanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].split('/')[-1].rstrip('.tar.gz'))
...
def submitBackupCreation(tempStoragePath, backupName, backupPath, backupDomain):
    try:
        ...
        ## /home/example.com/backup - backupPath
        ## /home/eaglepanel/1047.xml - metaPath
        ## /home/backup/<random_number> - CPHomeStorage
        ...
        schedulerPath = f'/home/eaglepanel/{backupDomain}-backup.txt'
        ...
        logging.EagleEPLogFileWriter.writeToFile(backupPath)
        logging.EagleEPLogFileWriter.writeToFile(tempStoragePath)

        execPath = "sudo nice -n 10 /usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/backupUtilities.py"
        ...
        if output.find('[5009]') > -1:
            logging.EagleEPLogFileWriter.writeToFile(output)
            writeToFile = open(schedulerPath, 'w')
        ...
        command = f'chown eaglepanel:eaglepanel {result[2]}'
        ProcessUtilities.executioner(command)
        ...
        for database in databases:
            ...
            dbName = database.find('dbName').text
            res = mysqlUtilities.mysqlUtilities.createDatabaseBackup(dbName, '/home/eaglepanel')
            if res == 0:
                ## This login can be further improved later.
                logging.EagleEPLogFileWriter.writeToFile('Failed to create database backup for %s. This could be false positive, moving on.' % (dbName))

            command = f'mv /home/eaglepanel/{dbName}.sql {CPHomeStorage}/{dbName}.sql'
            ProcessUtilities.executioner(command)
        ...
        ProcessUtilities.executioner(command, 'eaglepanel')
...
def cancelBackupCreation(backupCancellationDomain, fileName):
    try:
        ...
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(f"{str(msg)} [cancelBackupCreation]")
...
def main():
    parser = argparse.ArgumentParser(description='EaglePanel Backup Generator')
