...
from .EagleEPLogFileWriter import EagleEPLogFileWriter as logging
...
class ACLManager:
    ...
    def FindIfChild():
        try:
            ...
            config = json.loads(open('/home/eaglepanel/cluster', 'r').read())
    ...
    def CheckStatusFilleLoc(statusFile, domain=None):
        ...
        try:
            ...
            if domain != None:
                value = statusFile.split('eaglepanel/')[1]
                #logging.writeToFile(f'value of log file {value}')
                if value == f'{domain}_rustic_backup_log':
                    return 1
            return 0

        if (statusFile[:18] != "/home/eaglepanel/." or statusFile[:16] == "/home/eaglepanel" or statusFile[:4] == '/tmp' or statusFile[
                                                                                                                 :18] == '/usr/local/EagleEP') \
                and statusFile != '/usr/local/EagleEP/EagleEP/settings.py' and statusFile.find(
            '..') == -1 and statusFile != '/home/eaglepanel/.my.cnf' and statusFile != '/home/eaglepanel/.bashrc' and statusFile != '/home/eaglepanel/.bash_logout' and statusFile != '/home/eaglepanel/.profile':
            return 1
        else:
            return 0
    ...
    def CreateSecureDir():
        ...
        UploadPath = '/usr/local/EagleEP/tmp/'

        if not os.path.exists(UploadPath):
            command = 'mkdir %s' % (UploadPath)
            ProcessUtilities.executioner(command)

        command = 'chown eaglepanel:eaglepanel %s' % (UploadPath)
        ProcessUtilities.executioner(command)
    ...
    def GetServiceStatus(dic):
        ...
        if os.path.exists('/home/eaglepanel/pureftpd'):
            dic['ftpAsWhole'] = 1
        else:
            dic['ftpAsWhole'] = 0
