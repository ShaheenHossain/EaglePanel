...
import os.path
import shutil
from plogical import EagleEPLogFileWriter as logging
import subprocess
import argparse
import shlex
...
class mailUtilities:
    ...
    installLogPath = "/home/eaglepanel/openDKIMInstallLog"
    spamassassinInstallLogPath = "/home/eaglepanel/spamassassinInstallLogPath"
    RspamdInstallLogPath = "/home/eaglepanel/RspamdInstallLogPath"
    RspamdUnInstallLogPath = "/home/eaglepanel/RspamdUnInstallLogPath"
    eaglePanelHome = "/home/eaglepanel"
    mailScannerInstallLogPath = "/home/eaglepanel/mailScannerInstallLogPath"
    ...
    def SendEmail(sender, receivers, message):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg))
    @staticmethod
    def AfterEffects(domain):
        path = "/usr/local/EagleEP/install/rainloop/eaglepanel.net.ini"

        if not os.path.exists("/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/domains/"):
            os.makedirs("/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/domains/")

        finalPath = "/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/domains/" + domain + ".ini"
        finalPathJson = "/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/domains/" + domain + ".json"
...
""")
    ...
    def createEmailAccount(domain, userName, password, restore = None):
        try:
            ...
            execPath = "/usr/local/EagleEP/bin/python /usr/local/EagleEP/plogical/mailUtilities.py"
            ...
            #if not os.path.exists('/usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'):
            ...
            logging.EagleEPLogFileWriter.writeToFile(
    ...
    def setupDKIM(virtualHostName):
        try:
            ...
            command = 'chown eaglepanel:eaglepanel -R /usr/local/EagleEP/lib/python3.6/site-packages/tldextract/.suffix_cache'
            ProcessUtilities.executioner(command)

            command = 'chown eaglepanel:eaglepanel -R /usr/local/EagleEP/lib/python3.8/site-packages/tldextract/.suffix_cache'
            ProcessUtilities.executioner(command)

            command = 'chown eaglepanel:eaglepanel -R /usr/local/EagleEP/lib/python*/site-packages/tldextract/.suffix_cache'
            ...
            if not os.path.exists(keyTable):
                ...
                writeToFile.write("##### EaglePanel Generated File - Do not edit if you don't know what you are doing.\n")
                writeToFile.close()
...
"""
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [configureOpenDKIM]")
                print("0," + str(msg))
                return
            except BaseException as msg:
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [configureOpenDKIM]")
    ...
    def checkHome():
        try:
            try:
                ...
                if getpass.getuser() == 'root':
                    if not os.path.exists(mailUtilities.eaglePanelHome):
                        command = "mkdir " + mailUtilities.eaglePanelHome
                        subprocess.call(shlex.split(command), stdout=FNULL)

                    command = "sudo chown -R eaglepanel:eaglepanel " + mailUtilities.eaglePanelHome
                    subprocess.call(shlex.split(command), stdout=FNULL)
                else:
                    if not os.path.exists(mailUtilities.eaglePanelHome):
                        command = "mkdir " + mailUtilities.eaglePanelHome
                    ...
                    command = "chown -R eaglepanel:eaglepanel " + mailUtilities.eaglePanelHome
                    ProcessUtilities.executioner(command)
            except:
                FNULL = open(os.devnull, 'w')
                command = "chown -R eaglepanel:eaglepanel " + mailUtilities.eaglePanelHome
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [checkHome]")
    ...
    def installOpenDKIM(install, openDKIMINstall):
        try:
            ...
            if res == 1:
                ...
                writeToFile.close()
                logging.EagleEPLogFileWriter.writeToFile("[Could not Install OpenDKIM.]")
                return 0
            else:
                writeToFile = open(mailUtilities.installLogPath, 'a')
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[installOpenDKIM]")
    ...
    def restartServices():
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [restartServices]")
    ...
    def installSpamAssassin(install, SpamAssassin):
        try:
            ...
            if res == 1:
                ...
                writeToFile.writelines("Can not be installed.[404]\n")
                writeToFile.close()
                logging.EagleEPLogFileWriter.writeToFile("[Could not Install SpamAssassin.]")
                return 0
            else:
                writeToFile = open(mailUtilities.spamassassinInstallLogPath, 'a')
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[installSpamAssassin]")
...
"""
            ...
            if res == 1:
                ...
                writeToFile.writelines("Can not be installed.[404]\n")
                writeToFile.close()
                logging.EagleEPLogFileWriter.writeToFile("[Could not Install Rspamd.]")
                return 0
            else:
...
"""
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[installRspamd]")

    @staticmethod
    def uninstallRspamd(install, rspamd):
        from manageServices.serviceManager import ServiceManager
        try:
            logging.EagleEPLogFileWriter.writeToFile( "start................[uninstallRspamd]")
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[uninstallRspamd]")
    ...
    def changeRspamdConfig(install, changeRspamdConfig):
        try:
            ...
            tempfilepath = "/home/eaglepanel/tempfilerspamdconfigs"
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[changeRspamdConfig]")
    ...
    def changePostfixConfig(install , changePostfixConfig):
        try:
            tempfilepath = "/home/eaglepanel/tempfilepostfixconfigs"
            file = open(tempfilepath, "r")
            jsondata1 = file.read()
            jsondata = json.loads(jsondata1)
            file.close()
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[changePostfixConfig]")
    ...
    def changeRedisxConfig(install, changeRedisxConfig):
        try:
            tempfilepath = "/home/eaglepanel/saveRedisConfigurations"
            file = open(tempfilepath, "r")
            jsondata1 = file.read()
            jsondata = json.loads(jsondata1)
            file.close()
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[changeRedisxConfig]")
    ...
    def changeclamavConfig(install, changeclamavConfig):
        try:
            tempfilepath = "/home/eaglepanel/saveclamavConfigurations"
            file = open(tempfilepath, "r")
            jsondata1 = file.read()
            jsondata = json.loads(jsondata1)
            file.close()
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "[changeclamavConfig]")
    ...
    def installMailScanner(install, SpamAssassin):
        try:
            ...
            if mailUtilities.checkIfSpamAssassinInstalled():
                ...
                command = 'chmod +x /usr/local/EagleEP/CPScripts/mailscannerinstaller.sh'
                ProcessUtilities.executioner(command)


                command = '/usr/local/EagleEP/CPScripts/mailscannerinstaller.sh'
                ...
                if res == 1:
                    ...
                    writeToFile.writelines("Can not be installed.[404]\n")
                    writeToFile.close()
                    logging.EagleEPLogFileWriter.writeToFile("[Could not Install MailScanner.]")
                    return 0
                else:
                    writeToFile = open(mailUtilities.mailScannerInstallLogPath, 'a')
                ...
                writeToFile = open(mailUtilities.mailScannerInstallLogPath, 'a')
                writeToFile.writelines("Please install SpamAssassin from EaglePanel before installing MailScanner.[404]\n")
                writeToFile.close()
    ...
    def configureSpamAssassin():
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [configureSpamAssassin]")
            print("0," + str(msg))
            return
        except BaseException as msg:
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [configureSpamAssassin]")
        ...
        try:
            ...
            if install == '1':
                if not os.path.exists('/etc/systemd/system/cpecs.service'):
                    shutil.copy("/usr/local/EagleEP/postfixSenderPolicy/cpecs.service", "/etc/systemd/system/cpecs.service")
    ...
    def checkIfMailScannerInstalled():
        try:
            ...
            path = "/usr/local/EagleEP/public/mailwatch"
...
class MailServerManagerUtils(multi.Thread):
                ...
                for items in postFixLines:
                    if items.find('myhostname') > -1 and items[0] != '#':
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            logging.EagleEPLogFileWriter.writeToFile(f'Mail server SSL is issued with value: {self.mailHostName}')

                        self.MailSSL = 1
            except BaseException as msg:
                self.MailSSL = 0
                logging.EagleEPLogFileWriter.writeToFile('%s. [checkIfMailServerSSLIssued:864]' % (str(msg)))
    ...
    def RunServerLevelEmailChecks(self):
        try:
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Completed [200].')
    ...
    def install_postfix_dovecot(self):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Re-installing postfix..,10')
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Re-installing Dovecot..,15')
    ...
    def setup_email_Passwords(self, mysqlPassword):
        try:
            ...
            mysql_virtual_domains = "/usr/local/EagleEP/install/email-configs-one/mysql-virtual_domains.cf"
            mysql_virtual_forwardings = "/usr/local/EagleEP/install/email-configs-one/mysql-virtual_forwardings.cf"
            mysql_virtual_mailboxes = "/usr/local/EagleEP/install/email-configs-one/mysql-virtual_mailboxes.cf"
            mysql_virtual_email2email = "/usr/local/EagleEP/install/email-configs-one/mysql-virtual_email2email.cf"
            dovecotmysql = "/usr/local/EagleEP/install/email-configs-one/dovecot-sql.conf.ext"
    ...
    def setup_postfix_dovecot_config(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                self.centos_lib_dir_to_ubuntu("/usr/local/EagleEP/install/email-configs-one/master.cf", "/usr/libexec/",
                                              "/usr/lib/")
                self.centos_lib_dir_to_ubuntu("/usr/local/EagleEP/install/email-configs-one/main.cf",
                                              "/usr/libexec/postfix",
            ...
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_domains.cf",
                        "/etc/postfix/mysql-virtual_domains.cf")
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_forwardings.cf",
                        "/etc/postfix/mysql-virtual_forwardings.cf")
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_mailboxes.cf",
                        "/etc/postfix/mysql-virtual_mailboxes.cf")
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/mysql-virtual_email2email.cf",
                        "/etc/postfix/mysql-virtual_email2email.cf")
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/main.cf", main)
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/master.cf", master)
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/dovecot.conf", dovecot)
            shutil.copy("/usr/local/EagleEP/install/email-configs-one/dovecot-sql.conf.ext", dovecotmysql)
    ...
    def ResetEmailConfigurations(self):
        try:
            ...
            try:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.EagleEPLogFileWriter.writeToFile('%s. [setupConnection:75]' % (str(msg)))
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Resetting configurations..,40')
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Configurations reset..,70')
            ...
            logging.EagleEPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Fixing permissions..,90')

            self.fixEaglePanelPermissions()

            command = '/usr/local/EagleEP/bin/python /usr/local/EagleEP/dns/dnsManager.py ResetDNSConfigurations --tempStatusPath /home/eaglepanel/dnscheck'
            ProcessUtilities.executioner(command)

            command = 'touch /home/eaglepanel/postfix'
            ProcessUtilities.executioner(command)
