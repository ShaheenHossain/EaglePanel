...
class Upgrade:
    ...
    LogPathNew = '/home/eaglepanel/upgrade_logs'
    SoftUpgrade = 0
    ...
    @staticmethod
    def updateRepoURL():
        command = "sed -i 's|sgp.eaglepanel.sh|cdn.eaglepanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|lax.eaglepanel.sh|cdn.eaglepanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|fra.eaglepanel.sh|cdn.eaglepanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|mirror.eaglepanel.net|cdn.eaglepanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|sgp.eaglepanel.sh|cdn.eaglepanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|lax.eaglepanel.sh|cdn.eaglepanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|fra.eaglepanel.sh|cdn.eaglepanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|mirror.eaglepanel.net|cdn.eaglepanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)
    ...
    def fixSudoers():
        try:
            ...
            command = "chsh -s /bin/false eaglepanel"
    ...
    def download_install_phpmyadmin():
        try:
            ...
            try:
                shutil.rmtree("/usr/local/EagleEP/public/phpmyadmin")
            except:
                pass
...
"""
    ...
    def downoad_and_install_raindloop():
        try:
            ...
            # if os.path.exists("/usr/local/EagleEP/public/rainloop"):
            #
            #     if os.path.exists("/usr/local/lscp/eaglepanel/rainloop/data"):
            #         pass
            #     else:
            #         command = "mv /usr/local/EagleEP/public/rainloop/data /usr/local/lscp/eaglepanel/rainloop/data"
            #         Upgrade.executioner(command, 0)
            #
            #         command = "chown -R lscpd:lscpd /usr/local/lscp/eaglepanel/rainloop/data"
            #         Upgrade.executioner(command, 0)
            #
            #     iPath = os.listdir('/usr/local/EagleEP/public/rainloop/rainloop/v/')
            #
            ...
            if os.path.exists('/usr/local/EagleEP/public/snappymail'):
                shutil.rmtree('/usr/local/EagleEP/public/snappymail')

            while (1):
                command = 'unzip snappymail-%s.zip -d /usr/local/EagleEP/public/snappymail' % (Upgrade.SnappyVersion)
            ...
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            #             command = f'chown lscpd:lscpd /usr/local/lscp/eaglepanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
    ...
    def staticContent():
        ...
        command = "rm -rf /usr/local/EagleEP/public/static"
        ...
        shutil.move("/usr/local/EagleEP/static", "/usr/local/EagleEP/public/")
    ...
    def applyLoginSystemMigrations():
        try:
            ...
            connection, cursor = Upgrade.setupConnection('eaglepanel')

            try:
                cursor.execute(
                    'CREATE TABLE `baseTemplate_eaglepanelcosmetic` (`id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY, `MainDashboardCSS` longtext NOT NULL)')
            except:
                pass
  ...
  PRIMARY KEY (`id`)
    ...
    def GeneralMigrations():
        try:
            ...
            command = '/usr/local/EaglePanel/bin/python manage.py makemigrations'
            Upgrade.executioner(command, '/usr/local/EaglePanel/bin/python manage.py migrate', 0)
  ...
  CONSTRAINT `websiteFunctions_gdr_owner_id_4cf7983e_fk_websiteFu` FOREIGN KEY (`owner_id`) REFERENCES `websiteFunctions_gdrive` (`id`)
    ...
    'rootdb': {
        ...
        'PORT': '%s',
            ...
            settingsFile = '/usr/local/EagleEP/EagleEP/settings.py'
            ...
            command = 'git config --global user.email "support@eaglepanel.net"'

            if not Upgrade.executioner(command, command, 1):
                return 0, 'Failed to execute %s' % (command)

            command = 'git config --global user.name "EaglePanel"'
            ...
            if currentBranch.find('On branch %s' % (branch)) > -1 and currentBranch.find(
                ...
                if os.path.exists('EagleEP'):
                    shutil.rmtree('EagleEP')
    ...
    ### disable dkim signing in rspamd in ref to https://github.com/usmannasir/eaglepanel/issues/1176
    @staticmethod
    def FixRSPAMDConfig():
...
?>""" % (generate_pass())
            ...
            fileM = ['/usr/local/lsws/FileManager/', '/usr/local/EagleEP/install/FileManager',
                     '/usr/local/EagleEP/serverStatus/litespeed/FileManager',
            ...
            clScripts = ['/usr/local/EagleEP/CLScript/panel_info.py',
                         '/usr/local/EagleEP/CLScript/CloudLinuxPackages.py',
                         '/usr/local/EagleEP/CLScript/CloudLinuxUsers.py',
                         '/usr/local/EagleEP/CLScript/CloudLinuxDomains.py'
                , '/usr/local/EagleEP/CLScript/CloudLinuxResellers.py',
                         '/usr/local/EagleEP/CLScript/CloudLinuxAdmins.py',
    ...
    def someDirectories():
        ...
        command = "mkdir -p /usr/local/lscp/eaglepanel/logs"
        Upgrade.executioner(command, 0)
    ...
    def upgradeDovecot():
        try:
            ...
            configbackups = '/home/eaglepanel/configbackups'
...
"""
    ...
    def runSomeImportantBash():
        ...
        # Remove invalid crons from /etc/crontab Reference: https://github.com/usmannasir/eaglepanel/issues/216
        command = """sed -i '/EagleEP/d' /etc/crontab"""
        ...
        # Install any Eaglepanel missing crons to root crontab so its visible to users via crontab -l as root user
...
"""
            ...
            if data.find('IncScheduler.py') == -1:
                content = """
0 12 * * * /usr/local/EagleEP/bin/python /usr/local/EagleEP/IncBackups/IncScheduler.py Daily
...
'''
    ...
    def upgrade(branch):
        ...
        if not os.path.exists('/usr/local/lsws/bin/openlitespeed'):
            ...
            command = 'wget https://raw.githubusercontent.com/usmannasir/eaglepanel/stable/install/litespeed/httpd_config.xml'
            Upgrade.executioner(command, command, 0)
        ...
        if os.path.exists('/usr/local/EaglePanel.' + versionNumbring):
            os.remove('/usr/local/EaglePanel.' + versionNumbring)
        ...
        # ## Move static files
        #
        # imunifyPath = '/usr/local/EagleEP/public/imunify'
        #
        ...
        if os.path.exists(imunifyAVPath):
            execPath = "/usr/local/EagleEP/bin/python /usr/local/EagleEP/CLManager/CageFS.py"
            command = execPath + " --function submitinstallImunifyAV"
            Upgrade.executioner(command, command, 1)

            command = 'chmod +x /usr/local/EagleEP/public/imunifyav/bin/execute.py'
            Upgrade.executioner(command, command, 1)
