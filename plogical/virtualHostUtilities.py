...
class virtualHostUtilities:
    ...
    def OnBoardingHostName(Domain, tempStatusPath, skipRDNSCheck):
        ...
        logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Setting up hostname,10')
        ...
        if skipRDNSCheck:
            ...
            if os.path.exists('/home/eaglepanel/postfix'):
                pass
            else:
                message = 'This server does not come with postfix installed. [404]'
                print(message)
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, message)
                logging.EagleEPLogFileWriter.writeToFile(message)
                return 0
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            ...
            logging.EagleEPLogFileWriter.writeToFile(f'Postfix Hostname: {PostFixHostname}. Server IP {serverIP}. rDNS: {rDNS}, rDNS check {skipRDNSCheck}')
        ...
        if (CurrentHostName == PostFixHostname and CurrentHostName == rDNS) and os.path.exists(filePath):
            ...
            if SSLProvider == 'Denial':
                message = 'It seems that the hostname used with mail service and rDNS does not have a valid SSL certificate, EaglePanel will try to issue valid SSL and restart related services,20'
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Completed. [200]')
        else:
            ...
            try:
                ...
                try:
                    ...
                    logging.EagleEPLogFileWriter.writeToFile("Creating website result....%s" % coreResult1)
                    reutrntempath = coreResult1['tempStatusPath']
                    while (1):
                        lastLine = open(reutrntempath, 'r').read()
                        if os.path.exists(ProcessUtilities.debugPath):
                            logging.EagleEPLogFileWriter.writeToFile("Info web creating lastline ....... %s" % lastLine)
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Hostname SSL issued,50')
    ...
    def setupAutoDiscover(mailDomain, tempStatusPath, virtualHostName, admin):
        ...
        if mailDomain:
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Creating mail child domain..,80')
            ...
            result = virtualHostUtilities.createDomain(virtualHostName, childDomain, 'PHP 7.3', childPath, 1, 0, 0,
                                                       admin.userName, 0,
                                                       "/home/eaglepanel/" + str(randint(1000, 9999)))
    ...
    def createVirtualHost(virtualHostName, administratorEmail, phpVersion, virtualHostUser, ssl,
                          ...
                          tempStatusPath='/home/eaglepanel/fakePath', mailDomain=None, LimitsCheck=1):
        try:

            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Running some checks..,0')
            ...
            if LimitsCheck:
                ...
                if ACLManager.websitesLimitCheck(admin, 1) == 0:
                    logging.EagleEPLogFileWriter.statusWriter(tempStatusPath,
                ...
                if Websites.objects.filter(domain=virtualHostName).count() > 0:
                    logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'This website already exists. [404]')
                ...
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Creating DNS records..,10')
                ...
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Setting up directories..,25')
                ...
                if vhost.checkIfAliasExists(virtualHostName) == 1:
                    logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'This domain exists as Alias. [404]')
                    return 0, "This domain exists as Alias."

            postfixPath = '/home/eaglepanel/postfix'
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Creating configurations..,50')
            ...
            if ssl == 1:
                ...
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Setting up SSL..,70')
                if not os.path.exists(virtualHostUtilities.redisConf):
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'SSL set up done..,70')
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'vHost finalized..,70')
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'DKIM Setup..,70')
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Website successfully created. [200]')
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [createVirtualHost]")
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, str(msg) + " [404]")
            return 0, str(msg)
    ...
    def issueSSL(virtualHost, path, adminEmail):
        try:
            ...
            if retValues[0] == 0:
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(retValues[1]))
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + " [issueSSL]")
            print("0," + str(msg))
            return 0, str(msg)
    ...
    def issueSSLForHostName(virtualHost, path, skipLSCPDRestart=0):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [issueSSLForHostName]")
            print("0," + str(msg))
            return 0, str(msg)
    ...
    def createAlias(masterDomain, aliasDomain, ssl, sslPath, administratorEmail, owner=None):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [createAlias]")
            print("0," + str(msg))
    ...
    def issueAliasSSL(masterDomain, aliasDomain, sslPath, administratorEmail):
        try:
            ...
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [issueAliasSSL]")
            print("0," + str(msg))
    ...
    def deleteAlias(masterDomain, aliasDomain):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [deleteAlias]")
                print("0," + str(msg))
        else:
            try:
    ...
    def changeOpenBasedir(domainName, openBasedirValue):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [changeOpenBasedir]")
                print("0," + str(msg))
        else:
            try:
                confPath = virtualHostUtilities.Server_root + "/conf/vhosts/" + domainName
    ...
    def createDomain(masterDomain, virtualHostName, phpVersion, path, ssl, dkimCheck, openBasedir, owner, apache,
                     tempStatusPath='/home/eaglepanel/fakePath', LimitsCheck=1, alias = 0):
        try:
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'DKIM Setup..,30')
            ...
            if ssl == 1:
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Creating SSL..,50')
                if not os.path.exists(virtualHostUtilities.redisConf):
                    installUtilities.installUtilities.reStartLiteSpeed()
            ...
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Domain successfully created. [200]')
            return 1, "None"
    ...
    def switchServer(virtualHostName, phpVersion, server, tempStatusPath):
        try:
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Starting Conversion..,0')
            child = 0
            try:
            ...
            if server == virtualHostUtilities.apache:
                ...
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Creating apache configurations..,90')
                if child:
                ...
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Restarting servers and phps..,90')
                ...
                installUtilities.installUtilities.reStartLiteSpeed()
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Successfully converted.[200]')
            else:
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Starting Conversion..,0')
                ...
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Restarting server..,90')
                installUtilities.installUtilities.reStartLiteSpeed()
                logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, 'Successfully converted. [200]')

        except BaseException as msg:
            logging.EagleEPLogFileWriter.statusWriter(tempStatusPath, '%s[404]' % str(msg))
            logging.EagleEPLogFileWriter.writeToFile(str(msg) + "  [switchServer]")
    ...
    def permissionControl(path):
        try:
            command = 'sudo chown -R  eaglepanel:eaglepanel ' + path
    ...
    def EnableDisablePP(vhostName, username=None, password=None, path=None, wpid=None, externalApp=None):
        try:
            ...
            htpasstemp = f'/usr/local/EagleEP/{wpid}'
...
def main():
    ...
    if args.function == "createVirtualHost":
        ...
        try:
            ...
            tempStatusPath = '/home/eaglepanel/fakePath'
        ...
        virtualHostUtilities.setupAutoDiscover(1, '/home/eaglepanel/templogs', args.virtualHostName, admin)
