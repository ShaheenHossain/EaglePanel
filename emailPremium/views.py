    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'eaglepanelCleaner purgeLimitDomain ' + domainName + ' ' + str(operationVal)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'eaglepanelCleaner updateDomainLimit ' + domainName + ' ' + str(newLimit)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'eaglepanelCleaner purgeLimit ' + emailAddress + ' ' + str(operationVal)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'eaglepanelCleaner purgeLog ' + emailAddress + ' ' + str(operationVal)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'eaglepanelCleaner purgeLimitEmail ' + emailAddress + ' ' + str(monthlyLimit) + ' ' + str(
                    hourlyLimit)
                cacheClient.handleCachePurgeRequest(command)
        ...
        try:
            if request.method == 'POST':
                ...
                if installStatus.find("[200]") > -1:
                    ...
                    execPath = "export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/root/bin  && /usr/local/EagleEP/bin/python " + virtualHostUtilities.eaglePanel + "/plogical/mailUtilities.py"
        ...
        try:
            if request.method == 'POST':
                ...
                if mailUtilities.checkIfSpamAssassinInstalled() == 1:
                    ...
                    # logging.EagleEPLogFileWriter.writeToFile(str(data))
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                tempConfigPath = "/home/eaglepanel/" + str(randint(1000, 9999))
                    ...
                    if mailUtilities.checkIfRspamdInstalled() == 1:
                        ...
                        for items in data:
                            ...
                            if items.find('action =') > -1:
                                tempData = items.split(' ')
                                # logging.EagleEPLogFileWriter.writeToFile(str(tempData) + "action")
                                try:
                                    a = tempData[4]
                                except:
                                    a = tempData[2]
                        ...
                        for i in postdata:
                            if (i.find('smtpd_milters=') > -1 or i.find('smtpd_milters =') > -1) and i.find('non_smtpd_milters') < 0:
                                ...
                                if os.path.exists(ProcessUtilities.debugPath):
                                    logging.EagleEPLogFileWriter.writeToFile(f'smtpd_milters: {tempData}')
                                smtpd_milters = tempData.lstrip(' ')
                            if i.find('non_smtpd_milters=') > -1 or i.find('non_smtpd_milters =') > -1:
                                tempData = i.split('=')[1]

                                if os.path.exists(ProcessUtilities.debugPath):
                                    logging.EagleEPLogFileWriter.writeToFile(f'non_smtpd_milters: {tempData}')
                        ...
                        for i in postdata:
                            if i.find('write_servers =') > -1:
                                tempData = i.split(' ')
                                # logging.EagleEPLogFileWriter.writeToFile(str(tempData) + "redis")
                                write = tempData[2]
                                i = write.rstrip(";")
                                j = i.split('"')
                                write_servers = j[1]
                                # logging.EagleEPLogFileWriter.writeToFile(str(write_servers) + "write_servers")

                            if i.find('read_servers =') > -1:
                                tempData = i.split(' ')
                                # logging.EagleEPLogFileWriter.writeToFile(str(tempData) + "redis2")
                                read = tempData[2]
                                i = read.rstrip(";")
                                j = i.split('"')
                                read_servers = j[1]
                                # logging.EagleEPLogFileWriter.writeToFile(str(read_servers) + "read_servers")
...
def unistallRspamd(request):
    try:
        logging.EagleEPLogFileWriter.writeToFile("unistallRspamd...1")
        userID = request.session['userID']
        currentACL = ACLManager.loadedACL(userID)
    ...
    try:
        ...
        if ACLManager.CheckForPremFeature('email-debugger'):
            try:
                ...
                logging.EagleEPLogFileWriter.writeToFile("Result....3:" + str(msg))
        else:
            dic = {'status': 0, 'error_message': 'Kindly purchase email debugger Add-on'}
            json_data = json.dumps(dic)
            return HttpResponse(json_data)
...
def fixMailSSL(request):
    try:
        ...
        if ACLManager.CheckForPremFeature('email-debugger'):
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.EagleEPLogFileWriter.writeToFile("Result....3:" + str(res.content))
            return res
        else:
            dic = {'status': 0, 'error_message': 'Kindly purchase email debugger Add-on'}
            json_data = json.dumps(dic)
