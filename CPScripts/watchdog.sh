...
show_help() {
	echo -e "\nrun command: \e[31mnohup bash /etc/eaglepanel/watchdog.sh SERVICE_NAME >/dev/null 2>&1 &\e[39m"
	...
	echo -e "\nto exit watchdog , run command \e[31mbash /etc/eaglepanel/watchdog.sh kill\e[39m"
	echo -e "\n\nplease also create \e[31m/etc/eaglepanel/watchdog.flag\e[39m file with following format:"
...
check_service() {
	systemctl status $NAME 2>&1>/dev/null
		if [[ $? == "0" ]] ; then
			...
			if [ -f /etc/eaglepanel/watchdog.flag ] ; then
			flag="/etc/eaglepanel/watchdog.flag"
			LINE3=$(awk 'NR==3' $flag)
			LINE2=$(awk 'NR==2' $flag)
			LINE1=$(awk 'NR==1' $flag)
...
while [ true = true ]
	do
		if [[ $NAME == "pdns" ]] ; then
			if [ -f /home/eaglepanel/powerdns ] ; then
				check_service
			fi
		elif [[ $NAME == "postfix" ]] ; then
			if  [ -f /home/eaglepanel/postfix ] ; then
				check_service
			fi
		elif [[ $name == "pure-ftpd" ]] || [[ $name == "pure-ftpd-mysql" ]] ; then
			if [ -f /home/eaglepanel/pureftpd ] ; then
