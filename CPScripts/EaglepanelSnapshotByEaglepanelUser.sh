...
## Objective Find A Eaglepanel Users Domlogs Stats for last 5 days for all of their domains. v2
## https://gitlab.com/eaglepaneltoolsnscripts/snapshotbyeaglepaneluser
## How to use.
# ./EaglepanelSnapshotByEaglepanelUser.sh username
#./EaglepanelSnapshotEaglepanelUser.sh exampleuserbob
#
##bash <(curl -s https://gitlab.com/eaglepaneltoolsnscripts/snapshotbyeaglepaneluser/-/raw/master/EaglepanelSnapshotByEaglepanelUser.sh || wget -qO - https://gitlab.com/eaglepaneltoolsnscripts/snapshotbyeaglepaneluser/-/raw/master/EaglepanelSnapshotByEaglepanelUser.sh) exampleuserbob;
##
Username=$1
...
user_EaglepanelSnapshot="${Username}-EaglepanelSnapshot_${Now}.txt";

#create logfile in user's homedirectory.
#sudo touch "$user_EaglepanelSnapshot"

#chown logfile to user
#sudo chown ${Username}:${Username} "$user_EaglepanelSnapshot";
...
echo "Contents have been saved to ${user_EaglepanelSnapshot}"
}

# log everything, but also output to stdout
main_function 2>&1 | tee -a "${user_EaglepanelSnapshot}"