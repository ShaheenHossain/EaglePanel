...
## Objective Fix permissions issues on EaglePanel/cPanel/Plesk for a linux user or users
...
elif [ -f /usr/bin/eaglepanel ]; then
    	# EaglePanel check /usr/bin/eaglepanel
    	ControlPanel="eaglepanel"
...
helptext () {
    ...
    echo "-all: run on all Eaglepanel accounts"
    echo "--account or -a: specify a Eaglepanel/cPanel/Plesk account"
#   echo "--domain or -d: specify a Eaglepanel domain"
    tput sgr0
    exit 0
}
...
fixperms () {
	...
	elif [ "${ControlPanel}" == "eaglepanel" ] ; then

		fixperms_eaglepanel "${Username}"
		fixmailperms_eaglepanel
