...
## Objective Fix session issues on EaglePanel and standardized session paths.
# Fixes #430
# https://github.com/usmannasir/eaglepanel/issues/430
...
if [[ ! -e /usr/local/EagleEP/bin/cleansessions ]]; then
	touch /usr/local/EagleEP/bin/cleansessions
	chmod +x /usr/local/EagleEP/bin/cleansessions
	cat >> /usr/local/EagleEP/bin/cleansessions <<"EOL"
#!/bin/bash
...
command="/usr/local/EagleEP/bin/cleansessions >/dev/null 2>&1"
job="09,39 * * * * $command"
cat <(grep -i -v "$command" <(crontab -l)) <(echo "$job") | crontab -

echo "Checking cleansessions file"
cat /usr/local/EagleEP/bin/cleansessions
