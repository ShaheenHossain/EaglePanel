...
if [ -d "$DIR" ]; then
  ...
  echo "Please install SpamAssasin through the EaglePanel interface before proceeding"
  exit
fi
...
mkdir /usr/local/EagleEP/public/mailwatch

cd /usr/local/EagleEP/public/mailwatch

git clone --depth=1 https://github.com/mailwatch/MailWatch.git --branch 1.2 --single-branch

mv /usr/local/EagleEP/public/mailwatch/MailWatch/* /usr/local/EagleEP/public/mailwatch/

PASSWORD=$(cat /etc/eaglepanel/mysqlPassword)
USER=root
DATABASE=mailscanner
ADMINPASS=$(cat /etc/eaglepanel/adminPass)

### Fix a bug in MailWatch SQL File

sed -i 's/char(512)/char(255)/g' /usr/local/EagleEP/public/mailwatch/create.sql

##

mysql -u${USER} -p${PASSWORD} <"/usr/local/EagleEP/public/mailwatch/create.sql"
...
cp /usr/local/EagleEP/public/mailwatch/mailscanner/conf.php.example /usr/local/EagleEP/public/mailwatch/mailscanner/conf.php

sed -i "s/^define('DB_USER',.*/define('DB_USER','root');/" /usr/local/EagleEP/public/mailwatch/mailscanner/conf.php
sed -i "s/^define('DB_PASS',.*/define('DB_PASS','${PASSWORD}');/" /usr/local/EagleEP/public/mailwatch/mailscanner/conf.php
sed -i "s/^define('MAILWATCH_HOME',.*/define(\'MAILWATCH_HOME\', \'\/usr\/local\/EagleEP\/public\/mailwatch\/mailscanner');/" /usr/local/EagleEP/public/mailwatch/mailscanner/conf.php
...
cp /usr/local/EagleEP/public/mailwatch/MailScanner_perl_scripts/MailWatchConf.pm /usr/share/MailScanner/perl/custom/
...
ln -s /usr/local/EagleEP/public/mailwatch/MailScanner_perl_scripts/MailWatch.pm /usr/share/MailScanner/perl/custom
ln -s /usr/local/EagleEP/public/mailwatch/MailScanner_perl_scripts/SQLBlackWhiteList.pm /usr/share/MailScanner/perl/custom
ln -s /usr/local/EagleEP/public/mailwatch/MailScanner_perl_scripts/SQLSpamSettings.pm /usr/share/MailScanner/perl/custom
sed -i "s/^\$pathToFunctions =.*/\$pathToFunctions = '\/usr\/local\/EagleEP\/public\/mailwatch\/mailscanner\/functions.php';/" /usr/local/EagleEP/public/mailwatch/upgrade.php

/usr/local/lsws/lsphp72/bin/php /usr/local/EagleEP/public/mailwatch/upgrade.php
systemctl enable mailscanner
systemctl restart mailscanner

IPADDRESS=$(cat /etc/eaglepanel/machineIP)

### Furhter onwards is sieve configurations

#echo 'Setting up spamassassin and sieve to deliver spam to Junk folder by default'
##echo "If you wish mailscanner/spamassassin to send spam email to a spam folder please follow the tutorial on the Eaglepanel Website"
#echo 'Fix protocols'
