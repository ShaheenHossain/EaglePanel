...
class TestAPI(TestCase):
    ...
    def MakeRequest(self, endPoint, data):
        ...
        path = 'https://eaglepanel.xyz:8090/api/%s' % (endPoint)
    ...
    def test_submitWebsiteCreation(self):
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'domainName': 'api.eaglepanel.xyz', 'ownerEmail': 'usman@cyberpersons.com' , 'phpSelection': 'PHP 7.1',
        ...
        if self.MakeRequestRaw('http://api.eaglepanel.xyz').find('EaglePanel') > -1:
            exists = 1
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'domainName': 'api.eaglepanel.xyz'}
        ...
        if self.MakeRequestRaw('http://api.eaglepanel.xyz').find('404') > -1:
            exists = 1
    ...
    def test_changePackageAPI(self):
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'websiteName': 'eaglepanel.xyz', 'packageName': 'Default'}
    ...
    def test_submitWebsiteStatus(self):
        ...
        ## Suspend  check
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'websiteName': 'eaglepanel.xyz', 'state': 'Suspend'}
        ...
        if self.MakeRequestRaw('http://eaglepanel.xyz').find('404') > -1 or self.MakeRequestRaw(
                'http://eaglepanel.xyz').find('Access to this resource on the server is denied!') > -1:
            exists = 1
        ...
        suspend = 0
        import os
        if os.path.exists('/usr/local/lsws/conf/vhosts/eaglepanel.xyz-suspended'):
            suspend = 1
        ...
        data_ret = {'adminUser': 'admin', 'adminPass': '1234567', 'websiteName': 'eaglepanel.xyz', 'state': 'Unsuspend'}
        ...
        if self.MakeRequestRaw('http://eaglepanel.xyz').find('EaglePanel') > -1:
            exists = 1
        ...
        suspend = 0
        import os
        if os.path.exists('/usr/local/lsws/conf/vhosts/eaglepanel.xyz'):
            suspend = 1
